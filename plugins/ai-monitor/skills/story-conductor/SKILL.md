---
name: ai-monitor:story-issue-triage
...
- 再開:子subsystem起票 ラベル: !`echo "$AI_MONITOR_LABEL_RESUME_CREATE_SUBSYSTEM"`
...
- `add_labels_`:
  - !`echo "$AI_MONITOR_LABEL_CONFIRM_STORY_PR_INITIALIZER"`
...
- `body`: 空文字
- `labels`:
  - !`echo "$AI_MONITOR_LABEL_LAYER_SUBSYSTEM"`
  - !`echo "$AI_MONITOR_LABEL_CONFIRM_SUBSYSTEM_ISSUE_TRIAGE"`
...
- `remove_labels_`:
  - !`echo "$AI_MONITOR_LABEL_CONFIRM_STORY_ISSUE_TRIAGE"`
  - !`echo "$AI_MONITOR_LABEL_RESUME_CREATE_SUBSYSTEM"`
  - !`echo "$AI_MONITOR_LABEL_PHASE_END"`
- `add_labels_`:
  - （なし。役割を終えるので次ラベルなし）
