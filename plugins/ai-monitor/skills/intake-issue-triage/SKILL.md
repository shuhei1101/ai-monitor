---
name: ai-monitor:intake-issue-triage
...
- 複合ユースケースシナリオに影響あり → epic
  - `layer:*`: !`echo "$AI_MONITOR_LABEL_LAYER_EPIC"`
  - `確認:*`: !`echo "$AI_MONITOR_LABEL_CONFIRM_EPIC_ISSUE_TRIAGE"`
- 単一ユースケースシナリオに影響あり → story
  - `layer:*`: !`echo "$AI_MONITOR_LABEL_LAYER_STORY"`
  - `確認:*`: !`echo "$AI_MONITOR_LABEL_CONFIRM_STORY_ISSUE_TRIAGE"`
- 実装のみ（シナリオ不変）→ subsystem
  - `layer:*`: !`echo "$AI_MONITOR_LABEL_LAYER_SUBSYSTEM"`
  - `確認:*`: !`echo "$AI_MONITOR_LABEL_CONFIRM_SUBSYSTEM_ISSUE_TRIAGE"`
- 軽微な修正 → chore
  - `layer:*`: !`echo "$AI_MONITOR_LABEL_LAYER_CHORE"`
  - `確認:*`: !`echo "$AI_MONITOR_LABEL_CONFIRM_QUICK_IMPLEMENTER"`
...
- !`echo "$AI_MONITOR_LABEL_LAYER_INTAKE"`

#### type ラベル

以下から 1 つ判定して付与する:

- 既存の動作が仕様または期待と異なる問題の修正: !`echo "$AI_MONITOR_LABEL_TYPE_BUG"`
- 新機能追加・既存機能の有意な拡張: !`echo "$AI_MONITOR_LABEL_TYPE_FEAT"`
- 外部動作を変えずにコードを整理・改善: !`echo "$AI_MONITOR_LABEL_TYPE_REFACTOR"`
- ドキュメント・コメントのみの変更: !`echo "$AI_MONITOR_LABEL_TYPE_DOCS"`
- ビルド設定・依存更新・CI/CD など: !`echo "$AI_MONITOR_LABEL_TYPE_CHORE"`
- テストコードの追加・修正のみ: !`echo "$AI_MONITOR_LABEL_TYPE_TEST"`
...
- `is_pr`: false
- `remove_labels_`:
  - !`echo "$AI_MONITOR_LABEL_CONFIRM_INTAKE_ISSUE_TRIAGE"`
