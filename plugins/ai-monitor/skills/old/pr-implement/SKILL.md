...
| 動作 |
| --- |
| 採用方針の通りにコード変更（`pr-test-creator` が作成したテストが通るよう実装する） |
| 実装コードに合わせてテストコードを微調整する（大幅な書き直しは `pr-test-creator` の役割逸脱なので最小限に） |
...
`$AI_MONITOR_LABEL_CONFIRM_PR_REVIEW`（`確認:pr-reviewer`）ラベル付与と assignees 追加（`needs_user_review: true` の場合）は呼び出し側（`/ai-monitor:pr-implement-auto`）の責務。
...
| 新規ブランチ・新規 PR は作成しない |
| マージはしない |
| コンフリクトが出たら親に報告して停止（`-X ours/theirs` 禁止） |
