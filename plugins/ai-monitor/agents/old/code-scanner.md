...
テンプレート本文は `ai-monitor-tools` MCP の `template_get` で取得する（ラベル定数は Session Start フックで自動展開済み）。
...
## ステップ 7: `/ai-monitor:issue-create` スキルで起票

finding ごとに `/ai-monitor:issue-create` スキルを呼び出して起票する。
