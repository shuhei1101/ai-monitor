...
from ai_monitor.mcp.models import SearchResultItem
...
def test_normal_when_sorted(gh, resp, api):
    ...
    # 実行
    results = api.search_issues_and_prs("Issue", sort="created")
    # 検証
    kwargs = gh.rest.search.issues_and_pull_requests.call_args.kwargs
...
def test_normal_when_no_hit(gh, resp, api):
...
def test_error_when_api_error(gh, request_failed, api):
    ...
    with pytest.raises(RequestFailed):
        api.search_issues_and_prs("プロフィール編集")
