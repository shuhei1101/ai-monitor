...
def test_normal(gh, resp, api):
    ...
    # 実行
    res = api.create_review_comment(52, path="src/a.py", line=42, sender="architect", body="指摘です。")
    # 検証
...
def test_normal_when_multi_line(gh, resp, api):
    ...
    # 実行
    api.create_review_comment(52, path="src/a.py", line=48, start_line=42, sender="architect", body="範囲指摘。")
    # 検証
    kwargs = gh.rest.pulls.create_review_comment.call_args.kwargs
    assert kwargs["start_line"] == 42 and kwargs["line"] == 48


def test_error_when_out_of_diff(gh, resp, request_failed, api):
