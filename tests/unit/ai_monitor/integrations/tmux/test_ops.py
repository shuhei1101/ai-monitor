...
def test_interrupt(tmp_tmux_session):
    """中断キーの送信を確認する（正常系）。"""
    # 実行・検証: 例外を投げずに完了する（キーの並びは結合テストで確認する）
    ops.interrupt(tmp_tmux_session)


def test_interrupt_when_session_missing(tmp_session_name):
    """セッション不存在を確認する（異常系）。"""
    # 実行・検証
    with pytest.raises(subprocess.CalledProcessError):
        ops.interrupt(tmp_session_name)
