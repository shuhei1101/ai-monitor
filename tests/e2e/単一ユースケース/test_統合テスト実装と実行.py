...
def _assert_all_passed(data, level) -> None:
    """テスト結果表の結果列が全て ✅ であることを検証する。"""
    rows = result_rows((data.body or "").replace("\r\n", "\n"), section_name=level["section"])
    assert rows, "テスト結果表の行がない"
    for row in rows:
        assert "✅" in row, f"結果列が ✅ で埋まっていない: {row}"


def _assert_failed(gh_live, owner, repo, branch, seed_sha, data, level) -> None:
    """fail の記録と、実装 / テストコードを直していないことを検証する。"""
    rows = result_rows((data.body or "").replace("\r\n", "\n"), section_name=level["section"])
    assert [row for row in rows if "❌" in row], f"fail がテスト結果表に記録されていない: {rows}"
    ...
    assert level["test_path"] not in changed, "E2E テストコードが書き換えられている"


def test_normal_implement_single(
    ...
    """単一 UC シナリオの E2E テスト化と回帰対象の洗い出しを実環境で確認する（正常系・テスト実装）。"""
    owner, repo = repo_ctx
    ctx = setup_story(
        gh_live, owner, repo,
...
def test_normal_run_single(
    monitor, gh_live, repo_ctx, epic_issue_factory, epic_pr_factory, draft_pr_factory,
    story_issue_factory, commit_file, wait_until, sandbox,
):
    """レビュー済みテストの実行と結果記入を実環境で確認する（正常系・テスト実行・全 pass）。"""
    owner, repo = repo_ctx
    ctx = setup_story(
        gh_live, owner, repo,
        epic_issue_factory, epic_pr_factory, draft_pr_factory, story_issue_factory, commit_file,
        pr_body=STORY_PR_BODY_WITH_TABLE, files=story_branch_files(e2e_test=E2E_TEST_PY),
    )
    add_worktree(sandbox["local_path"], ctx["story_branch"])

    # 準備: 指揮役の実行指示 → 確認ラベル付与（起動トリガー）
    instruction = gh_live.rest.issues.create_comment(
        owner=owner, repo=repo, issue_number=ctx["pr"].number, body=RUN_INSTRUCTION
    ).parsed_data
    gh_live.rest.issues.add_labels(
        owner=owner, repo=repo, issue_number=ctx["pr"].number, labels=[f"確認:{SINGLE['tester']}"]
    )
    ...
    # 実行: テスト実行の完了を待つ
    data, report = _wait_handed_back(
        gh_live, owner, repo, ctx["pr"].number, wait_until, SINGLE, message="テスト実行の完了",
    )

    # 検証
    _assert_all_passed(data, SINGLE)
    assert server._is_minimized(instruction.node_id), "実行指示コメントが Resolve されていない"
    _assert_handed_back(data, report, SINGLE)


def test_normal_run_complex(
    monitor, gh_live, repo_ctx, epic_issue_factory, epic_pr_factory, commit_file, wait_until, sandbox,
):
    """レビュー済みテストの実行と結果記入を実環境で確認する（正常系・テスト実行・全 pass）。"""
    owner, repo = repo_ctx
    ctx = setup_epic(
        gh_live, owner, repo, epic_issue_factory, epic_pr_factory, commit_file,
        pr_body=EPIC_PR_BODY_WITH_TABLE, files=epic_branch_files(complex_e2e_test=COMPLEX_E2E_TEST_PY),
    )
    add_worktree(sandbox["local_path"], ctx["epic_branch"])

    # 準備: 指揮役の実行指示 → 確認ラベル付与（起動トリガー）
    instruction = gh_live.rest.issues.create_comment(
        owner=owner, repo=repo, issue_number=ctx["pr"].number, body=COMPLEX_RUN_INSTRUCTION
    ).parsed_data
    gh_live.rest.issues.add_labels(
        owner=owner, repo=repo, issue_number=ctx["pr"].number, labels=[f"確認:{COMPLEX['tester']}"]
    )
    ...
    # 実行: テスト実行の完了を待つ
    data, report = _wait_handed_back(
        gh_live, owner, repo, ctx["pr"].number, wait_until, COMPLEX, message="テスト実行の完了",
    )

    # 検証
    _assert_all_passed(data, COMPLEX)
    assert server._is_minimized(instruction.node_id), "実行指示コメントが Resolve されていない"
    _assert_handed_back(data, report, COMPLEX)


def test_error_fail_single(
    monitor, gh_live, repo_ctx, epic_issue_factory, epic_pr_factory, draft_pr_factory,
    story_issue_factory, commit_file, wait_until, sandbox,
):
    """fail の記録と指揮役への失敗報告を実環境で確認する（異常系・E2E テスト fail）。"""
    owner, repo = repo_ctx
    ...
    # 実行: 失敗報告までを待つ
    data, report = _wait_handed_back(
        gh_live, owner, repo, ctx["pr"].number, wait_until, SINGLE, message="fail の失敗報告",
    )

    # 検証
    _assert_failed(gh_live, owner, repo, ctx["story_branch"], seed_sha, data, SINGLE)
    _assert_handed_back(data, report, SINGLE)


def test_error_fail_complex(
    monitor, gh_live, repo_ctx, epic_issue_factory, epic_pr_factory, commit_file, wait_until, sandbox,
):
    """fail の記録と指揮役への失敗報告を実環境で確認する（異常系・E2E テスト fail）。"""
    owner, repo = repo_ctx
    ...
    # 実行: 失敗報告までを待つ
    data, report = _wait_handed_back(
        gh_live, owner, repo, ctx["pr"].number, wait_until, COMPLEX, message="fail の失敗報告",
    )

    # 検証
    _assert_failed(gh_live, owner, repo, ctx["epic_branch"], seed_sha, data, COMPLEX)
    _assert_handed_back(data, report, COMPLEX)
