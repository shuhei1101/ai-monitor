...
| epic-conductor | mock-reverse-engineer / complex-scenario-reverse-engineer / epic-poc-runner / mock-designer / complex-scenario-writer / story-conductor（PR 作成） / subsystem-conductor（PR 作成） / system-conductor | 各子への指示・タスク割り当て（PoC 再検証 / モック / シナリオ設計 / 統合テストの委任）・子 story PR 作成・複合 UC バグの subsystem への差し戻し・エスカレーションの判断と story への決定通知・親 system がある場合の上位への中継・epic → 親ブランチ マージの実行 |
