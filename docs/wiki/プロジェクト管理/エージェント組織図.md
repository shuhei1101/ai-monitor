...
入口は 2 つある。
プロジェクトの立ち上げ（新規 / 既存の移行とも）は system-architect、以降の機能開発は intake-issue-triager が受ける。
system-architect は `docs/wiki/` が未整備の状態で動くため intake を経由せず、ユーザーが直接 Issue を起票して確認ラベルを付ける。
...
| --- | --- | --- |
| system-architect | architecture-reverse-engineer・epic-conductor（起票） | RE PR での現状のアーキテクチャ図の作成依頼と完了報告の受領・承認済みエピック一覧からの子 epic 一括起票（以降の会話なし）。ユーザー起点で親を持たない |
| architecture-reverse-engineer | system-architect | 完了報告（発注元 = 依頼コメントの送信者） |
