...
親 PR の番号は本文に書かない（base を 1 段辿れば着く）。

### 着手順の表し方

同じ親の下に兄弟が複数ある場合、着手順は **確認ラベルを 1 本だけに付ける** ことで表す。

| 場面 | 操作 | 実施者 |
| --- | --- | --- |
| 子 PR をまとめて作るとき | 着手順が先頭の 1 本にだけ確認ラベルを付ける（後続は確認ラベルなしで作る） | 子を作る conductor |
| 先頭がマージされたとき | 次の着手順の兄弟へ確認ラベルを付ける | マージ完了報告を受けた親 conductor |

着手順の一覧は親 PR 本文が持つ（system PR の `## エピック一覧` の `着手順`、story PR の `## サブシステム一覧` の `依存`）。
親 conductor はそこを見て次の 1 本を決める。

確認ラベルが無い PR はモニターが起動しないため、これだけで直列化が成立する。
着手順に依存が無い兄弟は、最初から全員に確認ラベルを付けて並行に進めてよい。

GitHub の依存関係（blocked by）は Issue にしか張れないため、PR では使わない。

### PR スタックを使わない理由

GitHub の Stacked Pull Requests は「下の PR の head が上の PR の base」という一本の鎖しか表現できず、木構造を載せられない。

- 同じ base を持つ兄弟を 1 つのスタックに入れることはできない（分岐は鎖にならないため 422 になる）
- 鎖にするには後続の base を先行 PR の head へ付け替える必要があり、そうすると「base を 1 段辿ると親に着く」が崩れる
- さらに 1 つの PR が属せるスタックは 1 つだけなので、鎖に入れた PR はその配下の子 PR とスタックを組めなくなる

階層は base で辿れ、着手順は確認ラベルで表せるため、スタックが無くてもワークフローは成立する。
MCP ツール（`link_stack` / `unlink_stack`）は残っているが、手順書からは呼ばない。
...
| 実装・テスト | `{type}/{scope}/{ドメイン}/{UC名}/{変更内容}` | subsystem ブランチ | subsystem-conductor | tester → implementer（architect が割り当て） |

インターフェース定義とモジュール構成を分けるのは、それぞれにユーザーレビューが入るため。
片方の指摘でもう片方の PR が動かない形にする。

単体・結合テストと実装を 1 本にまとめるのは、テストが Red → 実装で Green という 1 続きの工程で、途中で面が変わると Red の実測結果を引き継げないため。
tester と implementer への割り当ては、同じ面の上で architect が順に行う（`プロジェクト管理/エージェント組織図.md` の連絡先一覧）。
...
テストと実装が同じ枝なのは、Red の実測から Green の実測までが 1 続きの工程だから（commit は先にテスト、次に実装の順で積む）。
