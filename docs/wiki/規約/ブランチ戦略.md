...
| --- | --- | --- | --- | --- |
| system | `docs/system/{ドメイン}` | `docs/system/aituber` | `master` | system → master マージ後 |
| epic | `{type}/epic/{ドメイン}` | `feat/epic/profile` | 親 system があれば system ブランチ、無ければ `master` | epic → 親ブランチ マージ後 |
| story | `{type}/story/{ドメイン}/{UC名}` | `feat/story/profile/edit` | epic ブランチ | story → epic マージ後 |
| subsystem | `{type}/{scope}/{ドメイン}/{UC名}` | `feat/backend/profile/edit` | story ブランチ | subsystem → story マージ後 |
...
- レイヤーブランチは変更内容スロットを省略する（そのレイヤー全体が対象なので分類までで一意）。変更内容スロットを持つのは配下の成果物ブランチ
...
| --- | --- | --- | --- | --- |
| 土台生成 | `docs/system/{ドメイン}/base` | system ブランチ | system-conductor | system-architect |
...
gitGraph
    ...
    branch feat/backend/profile/edit
    checkout feat/backend/profile/edit
    commit id: "subsystem の成果物"

    checkout feat/story/profile/edit
    merge feat/backend/profile/edit
...
config:
  gitGraph:
    mainBranchName: feat/epic/profile
---
gitGraph
...
config:
  gitGraph:
    mainBranchName: feat/story/profile/edit
---
gitGraph
...
config:
  gitGraph:
    mainBranchName: feat/backend/profile/edit
---
gitGraph
...
config:
  gitGraph:
    mainBranchName: docs/epic/profile
---
gitGraph
    commit id: "epic PR 作成"
