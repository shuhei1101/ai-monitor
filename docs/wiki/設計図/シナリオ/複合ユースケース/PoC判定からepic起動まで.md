...
| epic Issue の題材 | 成立条件が実測でしか決まらない機構を含む題材で書く | PoC 必要判定へ決定的に誘導 |
...
```mermaid
flowchart TD
  U0([ユーザー]) -->|epic Issue 起票 +<br>確認:epic-conductor 付与| UC1

  UC1([epic要件確定:正常シナリオ<br>（PoC 必要判定）]) -->|PoC Draft PR + 確認:epic-poc-runner +<br>指示コメント| UC2([実現可能性PoC検証:正常シナリオ])
  UC2 -->|epic Issue に PoC 結果 記録 +<br>確認:epic-conductor + 完了報告コメント| UC3([PoC結果確認:正常シナリオ<br>（画面変更なし）])
...
- epic Issue の `確認:*` が除去され、自分宛コメントが全て Resolve 済み
