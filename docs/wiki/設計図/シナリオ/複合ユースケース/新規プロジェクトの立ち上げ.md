...
```mermaid
flowchart TD
  U0([ユーザー]) -->|作りたいものを書いた system PR 起票 +<br>確認:system-conductor 付与| UC1([システム構成確定:正常シナリオ<br>（新規プロジェクト）])

  subgraph FOCUS["検証対象: 土台生成から epic 起票への引き継ぎ<br>（system PR は未マージのまま）"]
    UC2([システム土台生成:正常シナリオ])
    UC4([子epicPR作成:正常シナリオ])
    UC2 -->|土台の生成物を system ブランチに commit +<br>ラベル一括作成 + 完了報告 →<br>system PR に 確認:system-conductor| UC4
  end

  UC1 -->|構成とエピック一覧の確定 +<br>system PR 作成（base=master）+<br>確認:system-architect| UC2
  UC4 -->|先頭 epic に 確認:epic-conductor| DONE([epic PR 群が起票され<br>先頭 epic が epic-conductor の起動待ち])

  click UC1 "../単一ユースケース/システム構成確定.md#正常シナリオ新規プロジェクト"
  click UC2 "../単一ユースケース/システム土台生成.md#正常シナリオ"
  click UC4 "../単一ユースケース/子epicPR作成.md#正常シナリオ"
```

### 期待値

- system ブランチに `README.md`・`docs/rules.yaml`・`docs/wiki/` の骨格と `設計図/アーキテクチャ図.md` が反映され、全ディレクトリに `README.md` が併置されている
- `設計図/アーキテクチャ図.md` が system PR の `## 構成要件` と対応している
...
- エピック一覧と同数の epic PR が system PR に紐づいて存在し、`layer:epic` + `type:feat` が付与されている
- system PR の `## エピック一覧` の `対応 Issue` 列が全行 `#N` に更新されている
- `確認:epic-conductor` が着手順の先頭 epic にだけ付いている
- system PR に `確認:*` が 1 つも残っていない
