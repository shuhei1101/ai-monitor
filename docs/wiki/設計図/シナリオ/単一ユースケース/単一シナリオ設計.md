...
| No | セットアップ | 説明 | 補足 |
| --- | --- | --- | --- |
| 1 | story Draft PR | `確認:single-scenario-writer` 付与済み | 本文は `## 紐づく Issue` のみ |
| 2 | story Issue | ユースケース要件 確定済み | シナリオの元ネタ |
...
sequenceDiagram
  ...
  deactivate MON
  Note over MON: セッションは story PR close / merge まで常駐
```
