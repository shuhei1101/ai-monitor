...
architect（設計 Wiki の作成者・内部パイプラインの指揮役）が tester の作成したテストコードを設計 Wiki（結合 / モジュール構成）と照合し、設計どおりのテスト観点になっているかをレビューする単一ユースケース。
...
sequenceDiagram
  ...
  activate MON
  MON->>REPO: テストコードと設計 Wiki<br>（結合 / モジュール構成）を照合レビュー<br>（観点の過不足・粒度）
