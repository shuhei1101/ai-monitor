...
sequenceDiagram
  ...
  MON->>REPO: 設計図/非機能要件.md を作成<br>（外部の制約からアプリの決定値を確定・<br>決められない項目は 未確定 に残す）
...
- `設計図/非機能要件.md` の `## 決定値一覧` の各行に 外部の制約 / 出所 / アプリの決定値 / 根拠 が揃っている
- 銘柄未定など決められない項目が `## 未確定` に理由と決める条件つきで残っている
