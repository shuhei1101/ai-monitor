...
system-architect が確定した構成をもとに `設計図/アーキテクチャ図.md`・`docs/wiki/` の骨格・`docs/rules.yaml`・`README.md` の骨格・`.gitignore`・`.claude/settings.json` を system PR 上で生成し、ユーザー承認を経て system-conductor に完了報告する単一ユースケース。
...
sequenceDiagram
  ...
  loop 応答ループ（修正指示がある間）
    ...
    MON->>GH: ユーザーが指摘したコメントに<br>返信追記 +<br>system PR の assignee=ユーザー 再設定
    deactivate MON
  end
