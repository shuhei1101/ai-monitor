...
| No | セットアップ | 説明 | 補足 |
| --- | --- | --- | --- |
| 1 | epic Issue | `layer:epic` + `確認:epic-conductor` 付きで存在 | 親 intake Issue と Sub-issue リンク済み・本文は空 |
| 2 | assignee | 未設定 | モニター起動条件 |
| 3 | orchestrator | polling 中 | - |
| 4 | ユーザー回答 | 応答ループで PoC 不要・画面変更なしと回答する | 分岐を決定的に誘発（テストではユーザー役が固定回答） |
...
| No | セットアップ | 説明 | 補足 |
| --- | --- | --- | --- |
| 1 | 応答ループまで完了 | 5 セクション確定済み・`議論中` 除去済み（正常シナリオ（PoC 不要・画面変更なし）と同一の経過） | - |
| 2 | ユーザー回答 | PoC 不要・画面の新規作成 / レイアウト変更ありと回答済み | 分岐を決定的に誘発 |
...
| No | セットアップ | 説明 | 補足 |
| --- | --- | --- | --- |
| 1 | 正常シナリオの応答ループまで完了 | 5 セクション確定済み・`議論中` 除去済み | - |
| 2 | PoC 要否 | epic の成立が前例のない技術機構に依存し、ユーザーが PoC 必要と回答済み | 例: 未検証のプロトコル連携・性能が成立条件 |
...
sequenceDiagram
  ...
  MON->>GH: epic Issue の自分宛コメント一括 Resolve
  MON->>GH: epic Issue の 確認:epic-conductor 除去
  MON->>REPO: worktree + poc/{epic#35;}-{テーマ}<br>ブランチ作成 + 空 commit push
