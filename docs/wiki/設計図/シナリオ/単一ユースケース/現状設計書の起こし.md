...
| --- | --- | --- | --- | --- | --- |
| system | `architecture-reverse-engineer` | system-architect | `master` | `設計図/アーキテクチャ図.md` + 機能の洗い出し（完了報告コメント） | system-architect |
...
| subsystem | `ss-design-reverse-engineer` | subsystem-conductor | story ブランチ | `設計図/バックエンド結合/` / `設計図/フロントエンド結合/` / `設計図/モジュール構成/` / `設計図/ER図/` / `設計図/画面構成/` | architect |
...
sequenceDiagram
  ...
  activate MON
  MON-->>GH: 依頼コメントから<br>起こす対象ページと担当範囲を読む
