...
対応モニター: `complex-scenario-writer`
...
| No | セットアップ | 説明 | 補足 |
| --- | --- | --- | --- |
| 1 | epic Draft PR | `確認:complex-scenario-writer` 付与済み | 本文は `## 紐づく Issue` のみ |
| 2 | epic Issue | 5 セクション確定済み | ユースケース一覧が UC 名の SoT |
| 3 | 画面の方向性 | 画面変更ありの epic では epic PR の `## UI 設計` + モック確定済み | シナリオは確定済み画面を前提に書く |
