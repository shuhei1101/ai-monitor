---
template_version: 1.0.0
---

# エスカレーション対応

配下から「自レイヤーでは解決できない」と上がってきた論点を受けた conductor が、自レイヤーで打てる手の案と 1 段上位への中継を選択肢にしてユーザーへ確認し、選ばれた方針で下位へ再開指示を出す（または 1 段上位へ渡す）単一ユースケース。
どの段でも素通しせず必ずユーザー確認を挟む（そのレイヤーでしか気づけない解決策を取りこぼさないため）。
論点の種類は問わない（ライブラリの適合候補ゼロ・要件の矛盾・外部制約による UC 不成立 等）。
図は subsystem レベルで代表し、story / epic レベルは読み替え表で対応する。

対応エージェント: `subsystem-conductor` / `story-conductor` / `epic-conductor`

## 正常シナリオ（方針確認）

### セットアップ

| セットアップ | 説明 | 補足 |
| --- | --- | --- |
| Mock | なし（実環境で実行） | - |
| subsystem PR | `確認:subsystem-conductor` 付与済み + architect のエスカレーション報告コメント（経緯 + 論点・自分宛・未解決）あり | `確認:architect` は除去済み・`議論中` なし |
| 対応方針案コメント | 未投稿 | 未作業 |
| assignee | PR に未設定 | エージェント起動条件 |

### フロー

```mermaid
sequenceDiagram
  participant GH as GitHub
  participant ORC as モニター
  participant MON as subsystem-conductor

  Note over MON: 既存セッションを継続利用
  Note over GH: subsystem PR に<br>確認:subsystem-conductor 付与済み・<br>未解決のエスカレーション報告コメントあり
  ORC-->>GH: polling（確認ラベル + assignee なし を検知）
  ORC->>MON: 既存セッションへ送信（方針確認）
  activate MON
  MON->>GH: 論点を読み、自レイヤーで<br>打てる手があるかを検討
  MON->>GH: subsystem PR に対応方針案コメント<br>（自レイヤーの解決案 +<br>上位への中継 の選択肢）+<br>議論中 付与 + assignee=ユーザー 設定
  deactivate MON
  Note over MON: セッションは epic Issue close まで常駐
```

### 期待値

- subsystem PR に対応方針案コメント（自レイヤーの解決案 + 上位への中継 を並べた選択肢・ユーザー宛）が投稿されている
- subsystem PR に `議論中` + `assignee=ユーザー` が設定されている
- `確認:subsystem-conductor` は保持されたまま（選択後の実行で復帰するため）
- 上位 Issue へのラベル付与・コメント投稿がまだ発生していない（ユーザーの選択前に上げない）

## 正常シナリオ（自レイヤーで解決）

### セットアップ

| セットアップ | 説明 | 補足 |
| --- | --- | --- |
| Mock | なし（実環境で実行） | - |
| 方針確認まで完了 | 対応方針案コメント投稿済み・`議論中` + `assignee=ユーザー` で待機中 | 正常シナリオ（方針確認）と同一の経過 |
| ユーザー操作 | 自レイヤーの解決案を選ぶコメント + `議論中` 除去 + assignee 外し | 分岐を決定的に誘発 |

### フロー

```mermaid
sequenceDiagram
  actor U as ユーザー
  participant GH as GitHub
  participant ORC as モニター
  participant MON as subsystem-conductor

  U->>GH: subsystem PR に解決案の選択コメント +<br>議論中 除去 + assignee 外し
  ORC-->>GH: polling（議論中 除去 + assignee なし を検知）
  ORC->>MON: 既存セッションへ送信（実行）
  activate MON
  MON->>GH: エスカレーション報告コメントに<br>決定内容を返信追記して Resolve
  MON->>GH: subsystem PR に 確認:architect 付与 +<br>再開指示コメント<br>（決定した方針・@architect 宛）
  MON->>GH: subsystem PR の<br>確認:subsystem-conductor 除去
  deactivate MON
  Note over MON: 以降は通常の SS設計 フローに戻る
```

### 期待値

- エスカレーション報告コメントのスレッドに決定内容が返信追記され、Resolve 済み
- subsystem PR に `確認:architect` + 再開指示コメント（決定した方針・@architect 宛・未解決）が付与・投稿されている
- `確認:subsystem-conductor` が除去されている
- 上位 Issue へのラベル付与・コメント投稿が発生していない

## 正常シナリオ（上位への中継）

### セットアップ

| セットアップ | 説明 | 補足 |
| --- | --- | --- |
| Mock | なし（実環境で実行） | - |
| 方針確認まで完了 | 対応方針案コメント投稿済み・`議論中` + `assignee=ユーザー` で待機中 | 正常シナリオ（方針確認）と同一の経過 |
| ユーザー操作 | 上位への中継を選ぶコメント + `議論中` 除去 + assignee 外し | 分岐を決定的に誘発 |

### フロー

```mermaid
sequenceDiagram
  actor U as ユーザー
  participant GH as GitHub
  participant ORC as モニター
  participant MON as subsystem-conductor

  U->>GH: subsystem PR に中継を選ぶコメント +<br>議論中 除去 + assignee 外し
  ORC-->>GH: polling（議論中 除去 + assignee なし を検知）
  ORC->>MON: 既存セッションへ送信（実行）
  activate MON
  MON->>GH: エスカレーション報告コメントに<br>中継する旨を返信追記して Resolve
  MON->>GH: 親 story Issue に<br>確認:story-conductor 付与 +<br>中継コメント投稿（論点の要約 +<br>自レイヤーで解決できない理由 +<br>元コメントへのリンク・<br>@story-conductor 宛）
  MON->>GH: subsystem PR の<br>確認:subsystem-conductor 除去
  deactivate MON
  Note over MON: 担当面は確認ラベルなしで保留され、<br>上位の決定を待つ
```

### 期待値

- 親 story Issue に `確認:story-conductor` + 中継コメント（論点の要約 + 自レイヤーで解決できない理由 + 元コメントへのリンク・@story-conductor 宛・未解決）が付与・投稿されている
- エスカレーション報告コメントが Resolve 済み
- subsystem PR に `確認:*` が 1 つも残っていない（上位の決定まで保留）
- 論点のレイヤーを飛び越した Issue（epic Issue 等）へのラベル付与・コメント投稿が発生していない

## 正常シナリオ（シナリオ修正を伴う解決）

自レイヤーの解決案が設計シナリオの変更を伴う場合、決定を下位へ降ろす前に担当の scenario-writer へ修正を指示する。
図は story レベル（single-scenario-writer）で代表する。

### セットアップ

| セットアップ | 説明 | 補足 |
| --- | --- | --- |
| Mock | なし（実環境で実行） | - |
| 方針確認まで完了 | story Issue に対応方針案コメント投稿済み・`議論中` + `assignee=ユーザー` で待機中 | - |
| ユーザー操作 | 単一 UC シナリオの変更を伴う解決案を選ぶコメント + `議論中` 除去 + assignee 外し | 分岐を決定的に誘発 |

### フロー

```mermaid
sequenceDiagram
  actor U as ユーザー
  participant GH as GitHub
  participant ORC as モニター
  participant MON as story-conductor

  U->>GH: story Issue にシナリオ変更を伴う<br>解決案の選択コメント +<br>議論中 除去 + assignee 外し
  ORC-->>GH: polling（議論中 除去 + assignee なし を検知）
  ORC->>MON: 既存セッションへ送信（実行）
  activate MON
  MON->>GH: story Issue 本文の<br>ユースケース要件を更新
  MON->>GH: story PR に<br>確認:single-scenario-writer 付与 +<br>シナリオ修正の指示コメント<br>（決定した方針・@single-scenario-writer 宛）
  MON->>GH: 中継コメントに決定内容を返信追記して<br>Resolve
  MON->>GH: story Issue の 確認:story-conductor 除去
  deactivate MON
  Note over MON: 発生元 subsystem への決定通知は<br>シナリオ修正の完了報告を受けてから行う
```

### 期待値

- story Issue 本文が決定内容で更新されている
- story PR に `確認:single-scenario-writer` + シナリオ修正の指示コメント（@single-scenario-writer 宛・未解決）が付与・投稿されている
- 下位の subsystem Issue へのラベル付与・コメント投稿がまだ発生していない（シナリオ確定前に降ろさない）
- `確認:story-conductor` が除去されている

## 正常シナリオ（シナリオ修正完了後の決定通知）

### セットアップ

| セットアップ | 説明 | 補足 |
| --- | --- | --- |
| Mock | なし（実環境で実行） | - |
| story Issue | `確認:story-conductor` 付与済み + single-scenario-writer のシナリオ修正完了報告（エスカレーション対応由来・自分宛・未解決）あり | - |
| 単一 UC シナリオ | 修正 commit が story ブランチに積まれている | - |
| assignee | 未設定 | エージェント起動条件 |

### フロー

```mermaid
sequenceDiagram
  participant GH as GitHub
  participant ORC as モニター
  participant MON as story-conductor

  ORC-->>GH: polling（確認ラベル + assignee なし を検知）
  ORC->>MON: 既存セッションへ送信（決定通知）
  activate MON
  MON->>GH: 完了報告と修正 commit を照合
  MON->>GH: 完了報告コメントを Resolve
  MON->>GH: 発生元の subsystem Issue に<br>確認:subsystem-conductor 付与 +<br>決定通知コメント（決定した方針・<br>修正後のシナリオページ名・<br>@subsystem-conductor 宛）
  MON->>GH: story Issue の 確認:story-conductor 除去
  deactivate MON
  Note over MON: 以降は通常の subsystem 側フローに戻る
```

### 期待値

- single-scenario-writer の完了報告コメントが Resolve 済み
- 発生元の subsystem Issue に `確認:subsystem-conductor` + 決定通知コメント（決定した方針 + 修正後のシナリオページ名・@subsystem-conductor 宛・未解決）が付与・投稿されている
- `確認:story-conductor` が除去されている

## 正常シナリオ（上位の決定の受領）

### セットアップ

| セットアップ | 説明 | 補足 |
| --- | --- | --- |
| Mock | なし（実環境で実行） | - |
| subsystem PR | `確認:subsystem-conductor` 付与済み + story-conductor のエスカレーション決定コメント（決定内容・自分宛・未解決）あり | 上位への中継後に降りてきた状態 |
| assignee | PR に未設定 | エージェント起動条件 |

### フロー

```mermaid
sequenceDiagram
  participant GH as GitHub
  participant ORC as モニター
  participant MON as subsystem-conductor

  ORC-->>GH: polling（確認ラベル + assignee なし を検知）
  ORC->>MON: 既存セッションへ送信（決定の受領）
  activate MON
  MON->>GH: 決定コメントに受領を返信追記して Resolve
  MON->>GH: subsystem PR に 確認:architect 付与 +<br>再開指示コメント<br>（決定した方針・@architect 宛）
  MON->>GH: subsystem PR の<br>確認:subsystem-conductor 除去
  deactivate MON
  Note over MON: 以降は通常の SS設計 フローに戻る
```

### 期待値

- 上位の決定コメントのスレッドに受領が返信追記され、Resolve 済み
- subsystem PR に `確認:architect` + 再開指示コメント（決定した方針・@architect 宛・未解決）が付与・投稿されている
- `確認:subsystem-conductor` が除去されている

### 補足

- レイヤーごとの読み替え

  | レイヤー | 起動する面 | 報告元 | 自レイヤーの解決案の例 | 上位の中継先 | シナリオ修正の担当 | 再開指示の宛先 |
  | --- | --- | --- | --- | --- | --- | --- |
  | subsystem | subsystem PR | architect | 設計方針の変更・実装手段の代替 | 親 story Issue（`確認:story-conductor`） | architect 自身（SS設計で修正） | architect（`確認:architect`） |
  | story | story Issue | subsystem-conductor | 単一 UC の分割・subsystem の分け方の組み替え・UC 内での代替手段 | 親 epic Issue（`確認:epic-conductor`） | single-scenario-writer（`確認:single-scenario-writer`） | 該当 subsystem Issue（`確認:subsystem-conductor`） |
  | epic | epic Issue | story-conductor | epic 要件の変更・機能そのものの見直し・別 epic への切り出し | なし（終端） | complex-scenario-writer（`確認:complex-scenario-writer`） | 該当 story Issue（`確認:story-conductor`） |

- epic レベルは終端のため「上位への中継」の選択肢を出さない。
  代わりに、どの案でも解決できない場合はリセットの案内（ユーザーが対象 Issue に `確認:resetter` を付与する）を選択肢に含める
- 決定後は各レイヤーの通常フロー（設計 Wiki の修正・要件の再確定 等）にそのまま戻る

## 異常シナリオ

なし
