...
- レイヤーごとの読み替え
  ...
  | story | story Issue | subsystem-conductor | 単一 UC の分割・実装分担の組み替え・UC 内での代替手段 | 親 epic Issue（`確認:epic-conductor`） | single-scenario-writer（`確認:single-scenario-writer`） | 該当 subsystem Issue（`確認:subsystem-conductor`） |
