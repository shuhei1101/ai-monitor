...
sequenceDiagram
  ...
  MON->>GH: 親 story Issue に<br>中継コメント投稿（論点の要約 +<br>自レイヤーで解決できない理由 +<br>元コメントへのリンク・<br>@story-conductor 宛） +<br>確認:story-conductor 付与
  MON->>GH: subsystem PR の<br>確認:subsystem-conductor 除去
  deactivate MON
...
sequenceDiagram
  ...
  MON->>GH: 発生元の subsystem Issue に<br>決定通知コメント（決定した方針・<br>修正後のシナリオページ名・<br>@subsystem-conductor 宛） +<br>確認:subsystem-conductor 付与
  MON->>GH: story Issue の 確認:story-conductor 除去
  deactivate MON
