...
sequenceDiagram
  ...
  MON->>REPO: system ブランチ作成<br>（docs/system/{ドメイン}・base=master）+<br>空 commit push
  ...
  MON->>GH: system PR に完了報告コメントと<br>確認事項コメント（判断が割れる論点は<br>複数案の比較と推奨）を投稿
  ...
  MON->>REPO: 土台生成の成果物ブランチ作成<br>（docs/system/{ドメイン}/base・<br>base=system ブランチ）+ 空 commit push
...
| system ブランチ | `docs/system/{ドメイン}`（base=master）が作成済み | [リバースエンジニアリング起動](./リバースエンジニアリング起動.md) の成果物。新規に切らずこのブランチへ PR を作る |
| assignee | 未設定 | エージェント起動条件 |
...
sequenceDiagram
  ...
  MON->>GH: 概要 / 背景 / 構成要件 / エピック一覧 を<br>system PR 本文に反映
  MON->>GH: system PR に完了報告コメントと<br>確認事項コメント（実装と要件が乖離している箇所・<br>epic の切り方）を投稿
  MON->>GH: system PR に 議論中 付与 +<br>assignee=ユーザー 設定
  deactivate MON
