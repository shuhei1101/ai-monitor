...
sequenceDiagram
  ...
  MON->>GH: subsystem PR 本文のテスト結果表に<br>テストファイル名と変更種別の行を追加<br>（結果列は未記入）
...
sequenceDiagram
  ...
  MON->>GH: 起動のトリガーになった<br>割り当てコメントを Resolve
  MON->>GH: subsystem PR の 確認:tester 除去
  MON->>GH: subsystem PR に 完了報告コメント投稿<br>（既存の振る舞いを固定するテストと<br>あるべき構造を突くテストを区別して報告） +<br>確認:architect 付与
  deactivate MON
  Note over MON: セッションは epic Issue close まで常駐
```
