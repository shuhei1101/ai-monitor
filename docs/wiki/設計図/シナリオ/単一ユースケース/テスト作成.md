...
| --- | --- | --- |
| Mock | なし（実環境で実行） | - |
| 成果物 Draft PR | `確認:tester` 付与済み・設計 Wiki 確定済み・`## タスク一覧` 承認済み | - |
...
sequenceDiagram
  ...
  Note over GH: 成果物 PR に 確認:tester 付与済み
  ...
  MON->>GH: 親 subsystem PR 本文のテスト結果表に<br>テストファイル名と変更種別の行を追加<br>（結果列は未記入）
  MON->>GH: タスク一覧のテスト作成タスクに<br>チェックを入れる（自分がやった行だけ）
  MON->>GH: 起動のトリガーになった<br>割り当てコメントを Resolve
  MON->>GH: 成果物 PR の 確認:tester 除去
...
- テスト結果表にテストファイル名の行が追加されている（結果列は未記入）
- 追加した行の `変更種別` 列が全行埋まっている（新規作成したテストは `新規` / 既存テストを直したものは `変更` / 削除したテストは `削除`）
- `## タスク一覧` のテスト作成タスクがチェック済み（自分がやった行だけで、実装・テスト実行タスクは未チェック）
...
| 成果物 Draft PR | `確認:tester` 付与済み・設計 Wiki 確定済み・`## タスク一覧` 承認済み | 設計 Wiki はあるべき構造で書かれている |
| assignee | 未設定 | エージェント起動条件 |
...
sequenceDiagram
  ...
  MON->>GH: 起動のトリガーになった<br>割り当てコメントを Resolve
  MON->>GH: 成果物 PR の 確認:tester 除去
  MON->>GH: 成果物 PR に 完了報告コメント投稿<br>（既存の振る舞いを固定するテストと<br>あるべき構造を突くテストを区別して報告） +<br>確認:architect 付与
  deactivate MON
  Note over MON: セッションは epic PR マージまで常駐
```
