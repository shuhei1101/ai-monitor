---
template_version: 2.0.0
---

# テスト作成

tester が設計 Wiki（シナリオ / 結合 / モジュール構成）を元にテストコードを Red 状態で作成し、テスト結果表にテストファイル名を記入する単一ユースケース。
作成するのはテストコードとテスト用 fixture のみ（実装コード・データモデル・スタブは implementer の領分。実装ファイル不在による import エラーも Red として扱う）。

対応エージェント: `tester`

- 対応テストファイル: `tests/e2e/単一ユースケース/test_テスト作成.py`

## 正常シナリオ

### セットアップ

| セットアップ | 説明 | 補足 |
| --- | --- | --- |
| Mock | なし（実環境で実行） | - |
| subsystem Draft PR | `確認:tester` 付与済み・設計 Wiki 確定済み・`## タスク一覧` 承認済み | - |
| assignee | PR に未設定 | エージェント起動条件 |

### フロー

```mermaid
sequenceDiagram
  participant GH as GitHub
  participant ORC as モニター

  Note over GH: subsystem PR に 確認:tester 付与済み
  ORC-->>GH: polling（確認ラベル + assignee なし を検知）
  create participant MON as tester
  ORC->>MON: tmux セッション作成 +<br>フェーズドキュメント注入
  participant REPO as リポジトリ
  activate MON
  MON->>REPO: 設計 Wiki（シナリオ / 結合 /<br>モジュール構成）からテスト観点を把握
  MON->>REPO: テストコードを Red 状態で commit push<br>（既存テスト規約準拠）
  MON->>REPO: テスト実行（想定どおり fail することを確認）
  MON->>GH: subsystem PR 本文のテスト結果表に<br>テストファイル名の行を追加<br>（結果列は未記入）
  MON->>GH: subsystem PR の 確認:tester 除去
  MON->>GH: subsystem PR に 確認:architect 付与 +<br>完了報告コメント投稿<br>（自動報告・ユーザー承認なし）
  deactivate MON
  Note over MON: セッションは epic Issue close まで常駐
```

### 期待値

- テスト結果表にテストファイル名の行が追加されている（結果列は未記入・`## タスク一覧` のチェックは未変更）
- テストを実行すると想定どおり fail する（Red）
- commit に実装コード（データモデル・スタブ含む）が含まれていない（テストコードとテスト用 fixture のみ）
- subsystem PR に `確認:architect` + 完了報告コメント（未解決）が付与・投稿されている（`議論中` 付与なし）
- `確認:tester` が除去されている

## 正常シナリオ（リバースエンジニアリング）

### セットアップ

| セットアップ | 説明 | 補足 |
| --- | --- | --- |
| Mock | なし（実環境で実行） | - |
| `リバースエンジニアリング` ラベル | 対象の Issue / PR に付与済み | 本経路を選ぶ判定材料。ユーザーが system Issue に付け、子 Issue へ引き継がれる |
| subsystem Draft PR | `確認:tester` 付与済み・設計 Wiki 確定済み・`## タスク一覧` 承認済み | 設計 Wiki はあるべき構造で書かれている |
| assignee | PR に未設定 | エージェント起動条件 |
| 対象コード | 当該範囲が実装済み | テストが Green になる条件 |

### フロー

```mermaid
sequenceDiagram
  participant GH as GitHub
  participant ORC as モニター

  Note over GH: subsystem PR に 確認:tester 付与済み
  ORC-->>GH: polling（確認ラベル + assignee なし を検知）
  create participant MON as tester
  ORC->>MON: tmux セッション作成 +<br>フェーズドキュメント注入
  participant REPO as リポジトリ
  activate MON
  MON-->>REPO: 設計 Wiki（シナリオ / 結合 /<br>モジュール構成）からテスト観点を把握
  MON->>REPO: テストコードを commit push<br>（既存テスト規約準拠）
  MON->>REPO: テスト実行（現状の実装に対する<br>pass / fail を記録）
  MON->>GH: subsystem PR 本文のテスト結果表に<br>テストファイル名の行を追加<br>（結果列は未記入）
  MON->>GH: subsystem PR の 確認:tester 除去
  MON->>GH: subsystem PR に 確認:architect 付与 +<br>完了報告コメント投稿<br>（Green のテストと fail したテストを区別して報告）
  deactivate MON
  Note over MON: セッションは epic Issue close まで常駐
```

### 期待値

- テスト結果表にテストファイル名の行が追加されている（結果列は未記入・`## タスク一覧` のチェックは未変更）
- 設計 Wiki の全テスト観点に対応するテストが存在する
- 現状の実装で満たせている観点のテストは pass する（Green のまま受理する）
- あるべき構造との差分でまだ満たせない観点のテストは fail し、完了報告コメントで区別されている
- commit に実装コードの変更が含まれていない（テストコードとテスト用 fixture のみ）
- subsystem PR に `確認:architect` + 完了報告コメント（未解決）が付与・投稿されている（`議論中` 付与なし）
- `確認:tester` が除去されている

## 異常シナリオ（設計の見直しが必要）

### セットアップ

| セットアップ | 説明 | 補足 |
| --- | --- | --- |
| Mock | なし（実環境で実行） | - |
| テストコード作成の途中 | 設計 Wiki どおりに書けない構造問題を発見 | 例: 設計にないモジュール分割が必要 |

### フロー

```mermaid
sequenceDiagram
  participant GH as GitHub
  participant MON as tester

  MON->>MON: 設計 Wiki とテスト構造の不整合を検知
  MON->>GH: subsystem PR の 確認:tester 除去
  MON->>GH: subsystem PR に 確認:architect 付与 +<br>差し戻し報告コメント（理由・@architect 宛）
```

### 期待値

- subsystem PR に `確認:architect` + 差し戻し報告コメント（理由・未解決）が付与・投稿されている
- `確認:tester` が除去されている
