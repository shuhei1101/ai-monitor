...
修正用 PR の本文書式はサブシステム PR テンプレート（`テンプレート/PR本文/サブシステム.md`）に従う。
base は統合テストを実行しているブランチ（story レベル差し戻しでは story ブランチ・epic レベル差し戻しでは story ブランチ削除済みのため epic ブランチ）。
...
sequenceDiagram
  ...
  ORC->>MON: 既存セッションへ送信
  activate MON
  MON->>GH: subsystem Issue 本文の SA を更新
