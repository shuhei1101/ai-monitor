...
代わりにコンパクトを行わない。
Claude Code の `PreCompact` フックがコンパクトの開始をモニターへ知らせたうえでコンパクト自体をブロックし、モニターが tmux セッションごと作り直して起動時と同じプロンプトで立ち上げ直す。
...
- 対象セッションが同じ名前で作り直され、起動プロンプト（フェーズ本文・参考資料・Wiki 索引）で claude が立ち上がっている
- リセット後のターンで、エージェントが応答ループの手順どおりに動いている（フィードバックの本文反映 + `assignee=ユーザー` 再設定）
- `議論中` が保持されたままである（承認扱いにならない）

### 補足

- 起動し直しの確認は、エージェントの会話ログ（`~/.claude/projects/` 配下の jsonl）が新しく作られ、その最初のユーザーメッセージが起動プロンプトになっていることで行う
