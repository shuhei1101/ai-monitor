...
代わりにコンパクトを行わない。
Claude Code の `PreCompact` フックがコンパクトの開始をモニターへ知らせたうえでコンパクト自体をブロックし、モニターが `/clear` でセッションを空にしてから起動時と同じエージェントドキュメントを送り直す。
...
- 対象セッションへ `/clear` とエージェントドキュメント（フェーズ本文・参考資料・Wiki 索引）が順に送られている
