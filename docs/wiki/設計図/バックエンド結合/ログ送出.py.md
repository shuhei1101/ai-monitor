...
常駐プロセス（モニター / MCP サーバー）の composition root で観測基盤を初期化し、以降に標準 `logging` へ出力されたログを [OTel Collector](../コンテナ構成/observability.yaml.md#otel-collector)（OTLP gRPC）へ送出する。
...
| `OTEL_OTLP_ENDPOINT` | str | - | `http://localhost:4317` | Collector の OTLP gRPC 受付 URL | - | 環境変数 |
| `OTEL_OTLP_INSECURE` | bool | - | `true` | TLS を無効化するか | - | 環境変数 |
| `OTEL_DEPLOYMENT_ENVIRONMENT` | `"dev"` \| `"staging"` \| `"prod"` | - | `"dev"` | 環境名 | - | 環境変数 |
| `OTEL_SERVICE_NAMESPACE` | str | - | `"ai-monitor"` | サービス空間 | - | 環境変数 |
...
| 環境変数 | `OTEL_*` を未設定にする | 既定値での送出を確認する |
