...
新規セッションの起動プロンプトにはエージェントドキュメント（フェーズ + 参考資料 + Wiki 索引）の全文を載せる。
既存セッションへの再開送信では、ドキュメントは会話履歴に残っているため定型文とスナップショットだけを送る（コンテキストが上限に達した場合は[コンテキストリセット](./コンテキストリセット.py.md)が送り直す）。
...
sequenceDiagram
  ...
  MON->>GH: 処理中ラベルを付与
  MON->>TMUX: 起動プロンプト<br>（ドキュメント + 入力 +<br>スナップショット）を send-keys
```
