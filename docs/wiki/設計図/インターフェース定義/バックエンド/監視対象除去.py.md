...
エージェントが自セッションの監視面として登録した Issue / PR 番号を取り除く。
...
| `watch_numbers` | list[int] | ✅ | - | 監視面除去する Issue / PR 番号 | 1 件以上 | - |
...
sequenceDiagram
  ...
  participant IF as MCP ツール remove_watch_targets

  C->>IF: agent_name, number,<br>watch_numbers<br>（ヘッダに X-Project）
  IF->>IF: ヘッダから対象プロジェクトを解決
  IF->>IF: セッション台帳の監視面を除去
  IF-->>C: ok: true
```

### 期待値

- セッションの監視面番号一覧が除去後の状態になっている
- 台帳ファイルに永続化されている
- 戻り値が `ok: true`
