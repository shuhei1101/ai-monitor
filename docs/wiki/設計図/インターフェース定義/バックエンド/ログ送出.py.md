...
既定では OTel だけに送るが、コンソール出力のレベルを設定すると標準エラー出力にも同じログを出す。
tmux でプロセスの画面を見ているときに、動作状況をその場で追えるようにするため。
...
| `AI_MONITOR_OTEL_CONSOLE_LOG_LEVEL` | `"DEBUG"` \| `"INFO"` \| `"WARNING"` \| `"ERROR"` \| `"CRITICAL"` \| `null` | - | `null` | 標準エラー出力へ出すレベル | - | 環境変数。`null` なら出さない |
...
| 正常 | 正常系（コンソール出力あり） | コンソール出力のレベルを設定すると、OTel と標準エラー出力の両方に出る | 未設定なら OTel だけ |
...
| セットアップ | 説明 | 補足 |
| --- | --- | --- |
| Mock | OTel Collector（送信部の OTLP Exporter）を送出レコード記録用のスタブに差し替え | - |
| 環境変数 | `AI_MONITOR_OTEL_CONSOLE_LOG_LEVEL` に `WARNING` を設定する | 分岐を決定的に誘発 |
...
sequenceDiagram
  ...
  participant IF as 観測基盤
  participant COL as OTel Collector
  participant ERR as 標準エラー出力

  C->>IF: service_name
  IF->>IF: 設定読込 + 共通属性の組み立て
  IF->>IF: 3 Provider を配線し<br>標準 logging にハンドラを追加
  IF->>IF: コンソール出力のレベル指定があるため<br>標準エラー出力のハンドラも追加
  C->>IF: WARNING レベルのログを出力
  IF->>COL: 共通属性付きの LogRecord を送出
  IF->>ERR: 同じログを出力
```

### 期待値

- 標準エラー出力に、出力したログ本文が現れる
- 同じログが LogRecord として Collector にも送出されている（OTel 送出は止まらない）
- 設定したレベル未満のログは標準エラー出力に出ない（OTel には従来どおり INFO から送出される）
