...
リセット後の再開送信は heartbeat 周期が行う（[タイムアウト検知](./タイムアウト検知.py.md) と同じ周期）。

送信元は Claude Code の `StopFailure` フック。
`error_type` が `rate_limit` のときだけ発火するようマッチャーを絞る。
フックはエージェントのプロセス内で起動し、tmux 起動時に渡された環境変数（`AI_MONITOR_PROJECT` / `AI_MONITOR_AGENT` / `AI_MONITOR_NUMBER` / `AI_MONITOR_PORT`）から自分の素性を組み立て、フック入力の `transcript_path` を添えて本エンドポイントを叩く。

リセット時刻はフック入力の `error_message` ではなく会話ログから読む。
`error_message` に時刻が載る保証がないのに対し、会話ログには `You've hit your session limit · resets 2:30am (Asia/Tokyo)` の書式で必ず記録されるため。

上限はアカウント単位なので、1 セッションからの通知で全セッションを待機扱いにする。
