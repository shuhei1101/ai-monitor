...
各コメントには指摘箇所の周辺 diff（`diff_hunk`）が付くため、受け手はファイルを開き直さずに指摘対象のコードを読める。
...
sequenceDiagram
  ...
  T-->>GH: GraphQL でレビュースレッド一覧を取得<br>（path / start_line / line / isResolved /<br>コメント群 + 周辺 diff）
  T->>T: 解決済みスレッドを除外
  T->>T: 最後のコメントの<br>from / to で自分宛を判定
  T-->>A: threads（未解決のみ・is_addressed 付き）
```
