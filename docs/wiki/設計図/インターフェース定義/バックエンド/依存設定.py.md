...
親子関係（epic → story → subsystem）は Sub-issue リンクが持つため、本ツールでは扱わない。
兄弟間の順序（BE → FE、先行 epic → 後続 epic）や、別ツリーの Issue への依存を表すために使う。

依存先の状態は `get_issue_or_pr` の `blocked_by[].state` で取れるため、Issue 本文に依存の一覧や状態を書く必要はない。
