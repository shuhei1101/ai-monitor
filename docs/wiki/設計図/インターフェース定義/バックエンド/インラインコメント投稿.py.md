...
PR の diff 上の特定ファイル・特定行に紐づくレビューコメント（インライン指摘）を定型フォーマット（`> from: @{送信者}` + `> to: @{宛先}` + 本文 + 末尾の区切り線）で投稿する。会話欄のコメント（コメント投稿）とは別系統のスレッドになり、解決はレビュースレッド一括Resolve で行う。
...
- 対象ファイル・対象行に紐づくレビューコメントが 1 件追加され、本文が定型ブロック（`> from: @sender` + `> to: @receiver` + 本文 + 末尾の `---`）になっている
