...
指摘と返答が同じスレッドに並ぶため、どの指摘に対する対応かが GitHub UI 上で追える。

会話欄のコメントは 1 コメントを編集して `---` 区切りブロックを積む方式（`reply_comment`）で、本ツールはインライン指摘だけを対象にする。
どちらを使うかは返信先の種別で決まる（会話欄 = `reply_comment` / インライン = 本ツール）。
