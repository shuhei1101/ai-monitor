...
ユーザーが GitHub を見ていない時間帯でも、判断待ちや異常をチャットで拾えるようにするために使う。

エージェントが自分の言葉で送るための入口で、モニターが定型文を送る経路（[通知](../../モジュール構成/モニター/通知.py.md)）とは本文の作り手が違うだけで送出先は同じ。
エスカレーションのようにエージェントしか知り得ない文脈は本ツールで送る。

送信先は複数指定でき、`enabled` が `true` のものすべてに同じ本文を送る。
契機別の送出可否（`events`）はモニターが自分で送るときの判定なので、本ツールでは参照しない（エージェントが送ると決めたものは常に送る）。

有効な送信先が 1 つも無い場合は送信せず、送らなかったことを戻り値で返す（設定漏れを握り潰さない）。
