...
他エージェント同士のやり取りも含めて返すため、エージェントは面全体の経緯を読んだうえで応答できる。
自分宛の判定は最後のブロックで行い（to が `addressee`・to なしのユーザーブロック = 現担当宛・from が `addressee` = 自身の投稿、のいずれか）、結果は `is_addressed` に入れる。
判定の範囲は `規約/コメント.md` の精査対象表と一致させる（完了処理の一括 Resolve 対象は `is_addressed` が `true` のものに揃う）。
