...
2. ラベル名になるキーだけを残す
   - `AI_MONITOR_LABEL_` で始まらないキーを除く（テンプレートパス等）
   - `_COLOR_` / `_DESC_` を含むキーを除く（体裁の定義）
   - `_PREFIX` で終わるキーを除く（`確認:` / `処理中:` の接頭辞そのものでラベルではない）
3. 残ったキーの値をラベル名として、接頭辞から色と説明を決める
   - `確認:` で始まる場合、確認色と確認説明を使う
   - `処理中:` で始まる場合、処理中色と処理中説明を使う
   - `layer:` で始まる場合、レイヤー色とレイヤー説明を使う
   - `type:` で始まる場合、タイプ色とタイプ説明を使う
   - `優先度:` で始まる場合、優先度色と優先度説明を使う
   - `議論中` に完全一致する場合、議論中色と議論中説明を使う
   - `リバースエンジニアリング` に完全一致する場合、リバースエンジニアリング色と説明を使う
   - どれにも当てはまらない場合、`ValueError` を投げる
...
| --- | --- | --- | --- | --- | --- | --- |
| `test_build_label_specs` | 正常 | 全接頭辞と単独ラベルの色と説明が引ける | 各接頭辞と `議論中` / `リバースエンジニアリング` を 1 件ずつ含む定数を渡す | なし | 接頭辞ごとに対応する色と説明が入り、名前の昇順で返る | 7 分岐すべてを 1 ケースで通す |
