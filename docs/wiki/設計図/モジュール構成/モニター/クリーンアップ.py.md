...
1. epic 番号を起点に Sub-issue の子番号を再帰取得して集める（[Sub-issue 番号一覧](./GitHub連携.py.md#sub-issue-番号一覧)）
2. epic の親 Issue 番号（intake）を取得して加える（[親 Issue 番号取得](./GitHub連携.py.md#親-issue-番号取得)。`None` は加えない）
...
4. 処理中ラベルを除去 → tmux セッションを kill → 台帳から除去する（[ラベル除去](./GitHub連携.py.md#ラベル除去)・[セッション kill](./tmux連携.py.md#セッション-kill)・[除去](./エージェント管理.py.md#除去)。ラベル除去に失敗したセッションは見送り、次周期で再試行する）
