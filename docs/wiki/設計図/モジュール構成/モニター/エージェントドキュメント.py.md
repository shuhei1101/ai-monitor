...
モニターが起動プロンプトへ全文を載せ、コンテキストが上限に達したときは[コンテキストリセット](../../インターフェース定義/バックエンド/コンテキストリセット.py.md)を受けて送り直す。

読み元は既定でローカルのメインリポジトリ（ai-monitor 自身のクローンと、監視対象プロジェクトのクローン）。
push とキャッシュ待ちなしで反映するため。
worktree ではなくメインリポジトリを指す（worktree は作業用のブランチであり、フェーズや規約の正は master にあるため）。
読み元のベースは引数で受け取り、設定の読み出しは呼び出し側が行う。
ai-monitor 側とプロジェクト側でベースは独立した設定値なので、片方だけがローカルという組み合わせも成立する。

取得手段は場所の形から選ぶ（[ソース読み取り選択](../注入/URLドキュメント.py.md#ソース読み取り選択)）。
1 つのベース配下だけを辿る箇所はベースで 1 度選んだ [`ReadDoc`](../注入/URLドキュメント.py.md#ドキュメント読み取り型) を使い回し、対応表のように行ごとに読み先が変わる箇所は場所ごとに選ぶ。
対応表の行は絶対 URL を指せるため、ローカルのベース配下でもその 1 件だけネットワーク経由になる。
...
```python
"# 規約: コメント\n...\n\n# ai-monitor テンプレート: イシュー本文 / サブシステム\n..."
```
