...
方式を増やすときは設定クラスを追加して本型のユニオンに足し、[送出関数の組み立て](#送出関数の組み立て) に分岐を 1 つ加える。
...
| `user_gate` | [エージェント管理](./エージェント管理.py.md#ポーリング) | 対象番号 / 担当エージェント |
| `timeout_kill` | [クリーンアップ](./クリーンアップ.py.md#タイムアウト回収) | kill したセッション / 超過時間 |
| `epic_done` | [クリーンアップ](./クリーンアップ.py.md#最上位クローズの解放) | 完了した Issue 番号 / 解放したセッション数 |
