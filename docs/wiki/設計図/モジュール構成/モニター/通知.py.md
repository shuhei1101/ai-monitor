...
送出は補助手段なので、失敗しても本処理を止めない（例外を握らず戻り値で返し、呼び出し側はログに残すだけで先へ進む）。
エージェントが自分の言葉で送る経路は [通知送出](../../インターフェース定義/バックエンド/通知送出.py.md) が持ち、本ドメインは送出の実体と、モニターが検知する契機の定型文を担当する。
...
| 契機の通知 | 契機通知の組み立て | `features/notify/service.py` | 関数 | [`build_notifier`](#契機通知の組み立て) | 設定と送出先を束ねた通知関数を返す | composition root で 1 回だけ呼ぶ |
...
classDiagram
  ...
  class 契機通知型 {
    <<type>>
    +契機通知型(契機, 見出し, 本文) 送出結果
  }

  class 契機通知 {
    <<function>>
    +契機通知(契機, 見出し, 本文, 設定一覧, 送出先一覧) 送出結果
  }
  ...
  class ゲート通知 {
    <<function>>
    +ゲート通知(対象一覧, セッション一覧, 議論中ラベル, 契機通知) None
  }
...
| 値 | 論理名 | 説明 | 補足 |
| --- | --- | --- | --- |
| `rate_limit` | レートリミット到達 | Claude の利用上限に達して待機に入った | リセット時刻を載せる |
| `user_gate` | ユーザー確認ゲート | `議論中` + assignee が付いてユーザーの番になった | 同じ番号では 1 度だけ送る |
| `timeout_kill` | セッションのタイムアウト kill | 処理中のまま上限を超えたセッションを kill した | ハングの検知 |
| `epic_done` | epic 完了 | epic が close して配下のセッションを一括解放した | 完了の区切り |
...
設定と送出先は [契機通知の組み立て](#契機通知の組み立て) が束ねるため、検知側は契機と文面だけを渡す。
...
| 契機 | 検知する場所 | 文面に載せる値 |
| --- | --- | --- |
| `rate_limit` | [HTTP受信](./HTTP受信.py.md) の上限到達エンドポイント | 対象セッション / リセット時刻 |
| `user_gate` | [ゲート通知](#ゲート通知)（[周期駆動](./エージェント管理.py.md#周期駆動)が毎周期呼ぶ） | 対象番号 / 担当エージェント |
| `timeout_kill` | [クリーンアップ](./クリーンアップ.py.md#タイムアウト回収) | kill したセッション / 超過時間 |
| `epic_done` | [クリーンアップ](./クリーンアップ.py.md#一括解放) | 完了した Issue 番号 / 解放したセッション数 |

- `user_gate` は毎周期ゲートが開いたままになるため、[セッション](./エージェント管理.py.md#エージェントセッション) の通知済み番号で 1 度だけに絞る
...
```python
notify_open_gates(targets, sessions, discussion_label=labels.in_discussion, notify=notify)
```
...
| テスト名 | 正常/異常 | 概要 | 条件 | Mock | 期待値 | 補足 |
| --- | --- | --- | --- | --- | --- | --- |
| `test_notify_open_gates` | 正常 | ゲート開通の通知 | 議論中 + assignee 付きの監視面の対象 1 件 | 通知関数 | `user_gate` で 1 回通知され、通知済み番号に加わる | - |
