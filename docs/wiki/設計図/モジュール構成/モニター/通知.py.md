...
| ゲート通知 | サービス | `features/notify/gates.py` | 関数 | [`notify_open_gates`](#ゲート通知) | ユーザーの番になった対象をプロジェクトごとに 1 度だけ通知する | 通知済み番号は呼び出し側がプロジェクト単位で持つ |
| ゲート通知 | 内部処理 | `features/notify/gates.py` | 関数 | [`_confirm_agent`](#確認担当の解決) | 面の確認ラベルから手番を持つ担当名を求める | 無ければ `未割当` |
...
classDiagram
  ...
  class ゲート通知 {
    <<function>>
    +ゲート通知(対象一覧, 通知済み番号, プロジェクト, 議論中ラベル, 確認ラベル接頭辞, リポジトリ, 契機通知) None
  }
  class 確認担当の解決 {
    <<function>>
    +確認担当の解決(監視対象, 確認ラベル接頭辞) str
  }
...
| `user_gate` | ユーザー確認ゲート | `議論中` + assignee が付いてユーザーの番になった | [ゲート通知](#ゲート通知)（[周期駆動](./エージェント管理.py.md#周期駆動)が毎周期呼ぶ） | 対象番号 / 担当エージェント | 同じ番号では 1 度だけ送る（通知済み番号はプロジェクト単位で持つ） |
...
| `monitor_recovered` | モニターの復帰 | 再起動を打ち切っていたモニターの生存を監視役が確認した | [監視](./死活監視.py.md#監視)（監視役プロセス） | 対象名 | 打ち切り中だった場合にだけ送る（上限内の再起動では送らない） |
| `watchdog_recovered` | 監視役の復帰 | 再起動を打ち切っていた監視役の生存をモニターが確認した | [監視](./死活監視.py.md#監視)（モニタープロセス） | 対象名 | 打ち切り中だった場合にだけ送る（上限内の再起動では送らない） |
...
1 つの面を複数のセッションが監視面に持つため、記録はセッション単位ではなくプロジェクト単位で持つ（セッション単位だと同じ面が担当の数だけ届く）。
...
| 通知済み番号 | `notified` | `set[int]` | ✅ | - | 当該プロジェクトで通知済みの番号 | キーワード引数。破壊的に更新する。保持は[周期駆動](./エージェント管理.py.md#周期駆動)の呼び出し側 |
| プロジェクト | `project` | `str` | ✅ | - | 監視対象プロジェクト名 | キーワード引数。ログの識別に使う |
| 議論中ラベル | `discussion_label` | `str` | ✅ | - | ゲート開通の判定に使うラベル | キーワード引数。[ラベル設定](./エージェント管理.py.md#ラベル設定)の値 |
| 確認ラベル接頭辞 | `confirm_prefix` | `str` | ✅ | - | 担当名を引く確認ラベルの接頭辞 | キーワード引数。[ラベル設定](./エージェント管理.py.md#ラベル設定)の値 |
...
| 型 | 説明 | 補足 |
| --- | --- | --- |
| `None` | なし | `notified` を破壊的に更新する |

#### 処理

1. `targets` から 議論中ラベルあり + assignee ありの対象を集める（ユーザーの番になっている対象）
2. 閉じたゲートの通知済み記録を落とす（次に開いたときへ再通知できるようにする）
3. 未通知の番号を昇順に通知して、通知済みへ加える
   - 担当名は対象の確認ラベルから引く（[確認担当の解決](#確認担当の解決)）
...
| テスト名 | 正常/異常 | 概要 | 条件 | Mock | 期待値 | 補足 |
| --- | --- | --- | --- | --- | --- | --- |
| `test_notify_open_gates` | 正常 | ゲート開通の通知 | 議論中 + assignee + 確認ラベル付きの対象 1 件 | 通知関数 | `user_gate` で 1 回通知され、確認ラベル由来の担当名・リポジトリ・対象番号が渡り、通知済みへ加わる | - |
...
| `test_notify_open_gates_when_multiple_watchers` | 正常 | 複数セッション監視時の重複防止 | 同じ対象で 2 周期分呼ぶ | 通知関数 | 通知は 1 回だけ | 記録がプロジェクト単位であることの確認 |
| `test_notify_open_gates_when_no_confirm_label` | 正常 | 確認ラベルなしの担当表示 | ユーザーが手で 議論中 + assignee を付けた対象 | 通知関数 | 担当が `未割当` で通知される | 分岐の確認 |
| `test_notify_open_gates_when_not_open` | 正常 | ゲート未開通の除外 | 議論中はあるが assignee が無い対象 | 通知関数 | 通知されない | 分岐の確認 |

---

### 確認担当の解決
> 物理名: `_confirm_agent`<br>
> 種別: 関数
...
| 論理名 | 引数名 | 型 | 必須 | デフォルト | 説明 | 補足 |
| --- | --- | --- | --- | --- | --- | --- |
| 監視対象 | `target` | [`_Gateable`](#ゲート判定対象) | ✅ | - | 担当を引く対象 | - |
| 確認ラベル接頭辞 | `confirm_prefix` | `str` | ✅ | - | 確認ラベルの接頭辞 | - |

#### 戻り値

| 型 | 説明 | 補足 |
| --- | --- | --- |
| `str` | 担当エージェント名 | 確認ラベルが無ければ `未割当` |

#### 処理

1. 接頭辞で始まるラベルから接頭辞を除いた名前を集める
2. 名前を ` / ` で連結して返す（規約上 1 面 1 件だが、増えていたら気づけるよう全て見せる）
3. 1 件も無ければ `未割当` を返す
...
なし（同一ファイルの[ゲート通知](#ゲート通知)の単体テストで実物のまま検証する）
