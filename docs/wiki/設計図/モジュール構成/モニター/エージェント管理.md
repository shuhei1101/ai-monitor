...
| 共通 | スナップショット組み立て | `features/agents/service.py` | 関数 | [`build_context_snapshot`](#スナップショット組み立て) | 対象の系譜と状態を機械収集して整形 | 送信ペイロードに毎回添付 |
...
| 親番号 | `parent_number` | `int \| None` | 公開 | `None` | Sub-issue リンクの親 Issue 番号 | `12` | - |
| 子番号一覧 | `sub_issue_numbers` | `list[int]` | 公開 | `[]` | Sub-issue リンクの子 Issue 番号 | `[36, 37]` | intake 自動 close 判定に使用 |
...
| intake-issue-triager 確認ラベル | `confirm_intake_issue_triage` | [`LabelName`](#ラベル名) | 公開 | - | `AI_MONITOR_LABEL_CONFIRM_INTAKE_ISSUE_TRIAGE` の値 | `"確認:intake-issue-triager"` | - |
...
| intake-issue-triager 処理中ラベル | `processing_intake_issue_triage` | [`LabelName`](#ラベル名) | 公開 | - | `AI_MONITOR_LABEL_PROCESSING_INTAKE_ISSUE_TRIAGE` の値 | `"処理中:intake-issue-triager"` | - |
...
| --- | --- | --- | --- | --- | --- | --- |
| `test_settings_loads_yaml` | 正常 | yaml からの読み込み | 全キーを書いた settings.yaml | なし | 各プロパティが対応する値になる | - |
| `test_settings_env_overrides_yaml` | 正常 | 同名環境変数の上書き | yaml の `port` + 同名の環境変数 | なし | 環境変数の値が優先される | - |
| `test_settings_env_file_overrides` | 正常 | 環境差分ファイルの上書き | `AI_MONITOR_ENV=e2e` + `settings.e2e.yaml` に同名キー | なし | 環境差分の値が共通より優先される | - |
| `test_settings_missing_token_raises` | 異常 | `github_token` 未設定 | `github_token` を消した settings.yaml | なし | バリデーションエラー | - |
...
| テスト名 | 正常/異常 | 概要 | 条件 | Mock | 期待値 | 補足 |
| --- | --- | --- | --- | --- | --- | --- |
| `test_find_returns_matching_session` | 正常 | 主番号での検索 | 別プロジェクトの同番号を含む登録済み 3 件から検索 | なし | 一致する 1 件を返す | - |
| `test_find_matches_watch_number` | 正常 | 監視面での検索 | 監視面に 60 を持つセッション + `number=60` | なし | 該当セッションを返す | - |
| `test_find_returns_none_when_missing` | 正常 | 未登録は None | 空の台帳 | なし | `None` | - |
...
| テスト名 | 正常/異常 | 概要 | 条件 | Mock | 期待値 | 補足 |
| --- | --- | --- | --- | --- | --- | --- |
| `test_register_adds_and_persists` | 正常 | 追加と永続化 | 新規セッション 1 件 | save_sessions | 一覧に追加され `save_sessions` が呼ばれる | - |
...
| テスト名 | 正常/異常 | 概要 | 条件 | Mock | 期待値 | 補足 |
| --- | --- | --- | --- | --- | --- | --- |
| `test_touch_updates_last_seen` | 正常 | 生存時刻の更新 | 登録済みセッション | save_sessions | `last_seen_at` が更新され永続化される | - |
...
| テスト名 | 正常/異常 | 概要 | 条件 | Mock | 期待値 | 補足 |
| --- | --- | --- | --- | --- | --- | --- |
| `test_add_watch_appends_and_persists` | 正常 | 追加と永続化 | 登録済みセッション + 新規番号 2 件 | save_sessions | 監視面に追加され `save_sessions` が呼ばれる | - |
| `test_add_watch_ignores_duplicates` | 正常 | 登録済み番号の無視 | 既に監視面にある番号を追加 | save_sessions | 一覧が重複しない | - |
| `test_add_watch_missing_session_raises` | 異常 | セッション不明 | 台帳に無い主番号 | save_sessions | `KeyError`・台帳は不変 | 例外表「セッションが台帳にない」に対応 |
...
| テスト名 | 正常/異常 | 概要 | 条件 | Mock | 期待値 | 補足 |
| --- | --- | --- | --- | --- | --- | --- |
| `test_remove_watch_removes_and_persists` | 正常 | 除去と永続化 | 監視面に 60, 61 を持つセッションから 60 を除去 | save_sessions | 監視面が `[61]` になり永続化される | - |
| `test_remove_watch_ignores_missing_number` | 正常 | 未登録番号の無視 | 監視面に無い番号を除去 | save_sessions | 一覧は不変（例外にしない） | - |
...
| テスト名 | 正常/異常 | 概要 | 条件 | Mock | 期待値 | 補足 |
| --- | --- | --- | --- | --- | --- | --- |
| `test_release_by_number_removes_matching` | 正常 | プロジェクト + 主番号一致の除去 | 別プロジェクトの同番号を含む台帳から主番号 35 で解放 | save_sessions | 対象プロジェクトの分だけ除去 + 除去分を返す + 永続化 | - |
| `test_release_by_number_returns_empty_when_missing` | 正常 | 一致なしは空リスト | 主番号 99 で解放 | save_sessions | `[]`・台帳は不変 | - |
...
| テスト名 | 正常/異常 | 概要 | 条件 | Mock | 期待値 | 補足 |
| --- | --- | --- | --- | --- | --- | --- |
| `test_poll_filters_label_and_assignee` | 正常 | 確認ラベル + assignee なしの絞り込み | ラベル / assignee の有無が混在する `targets` | GitHub API / tmux | 条件一致の対象だけが処理される | - |
| `test_poll_creates_session_for_new_target` | 正常 | 新規対象にセッション作成 + skill 起動 | 対象 1 件を含む `targets`・台帳に未登録 | GitHub API / tmux | tmux 新規作成 + 台帳に登録 + skill 起動文字列を送信 | - |
| `test_poll_sends_to_existing_session` | 正常 | 既存セッションへ send-keys | 対象 1 件を含む `targets`・台帳に登録済み | GitHub API / tmux | 既存セッションへ送信・新規作成なし | - |
| `test_poll_skips_processing_target` | 正常 | 処理中ラベル付きの対象を除外 | `targets` の対象に処理中ラベルあり | GitHub API / tmux | 送信・ラベル操作なし | - |
| `test_poll_processes_in_priority_order` | 正常 | 優先度ソート順に処理 | 優先度ラベルの異なる対象 2 件を含む `targets` | GitHub API / tmux | 高優先度 → 低優先度の順で `_process_one` | - |
...
| テスト名 | 正常/異常 | 概要 | 条件 | Mock | 期待値 | 補足 |
| --- | --- | --- | --- | --- | --- | --- |
| `test_build_skill_command_format` | 正常 | 起動文字列の組み立て | `agent.name="architect"`・`number=52` | なし | `"/ai-monitor:architect 52"` | - |
...
| テスト名 | 正常/異常 | 概要 | 条件 | Mock | 期待値 | 補足 |
| --- | --- | --- | --- | --- | --- | --- |
| `test_process_one_toggles_processing_label` | 正常 | 送信前後の処理中ラベル付け外し | 対象 1 件 | GitHub API / tmux | 送信前に処理中ラベル付与・送信文 = 定型文 + スナップショット | - |
...
| テスト名 | 正常/異常 | 概要 | 条件 | Mock | 期待値 | 補足 |
| --- | --- | --- | --- | --- | --- | --- |
| `test_sort_key_orders_by_rank_then_number` | 正常 | 同ランクは番号昇順 | 同ランク・番号違いの対象 2 件 | なし | （ランク, 番号）のタプルで昇順 | - |
...
対象の系譜（祖先 / 子孫 Issue・紐づく PR・派生 PoC PR）と各ノードの state / ラベル / assignee を機械収集して整形する。
解釈・フェーズ判断は含めない。
...
| 論理名 | 引数名 | 型 | 必須 | デフォルト | 説明 | 補足 |
| --- | --- | --- | --- | --- | --- | --- |
| 監視対象 | `target` | [`MonitorTarget`](#監視対象) | ✅ | - | 系譜収集の起点 | - |
| 対象一覧 | `open_targets` | [`list[MonitorTarget]`](#監視対象) | ✅ | - | 周期の open 対象一覧（PR のぶら下げに再利用） | - |
...
```python
"epic #35 [open] labels=[layer:epic] assignees=[]\n  └ story #36 [open] ..."
```
...
1. 起点の Issue を確定する
   - Issue の場合、そのまま起点にする
   - PR の場合、`linked_issue_numbers`（本文 `## 紐づく Issue`）の Issue を起点にする
2. 親番号を再帰的に辿り、最上位の祖先（intake Issue）まで遡る（[Issue 取得](./GitHub連携.md#issue-取得)）
3. 最上位の祖先から Sub-issue リンクを再帰的に下り、兄弟のサブツリーを含む全子孫 Issue を収集する（[Issue 取得](./GitHub連携.md#issue-取得)）
4. `open_targets` に含まれる PR を `linked_issue_numbers` で対応する Issue の配下にぶら下げる（PoC PR もここで拾われる）
5. 各ノードに state / ラベル / assignee を付けたツリー文字列に整形して返す
...
| セットアップ | 説明 | 補足 |
| --- | --- | --- |
| 系譜ツリー | intake 配下に epic 2 本・epic1 配下に story → subsystem、各階層の PR と PoC PR を持つ GitHub API の Mock 応答一式 | fixture 名 `lineage_tree` |

| テスト名 | 正常/異常 | 概要 | 条件 | Mock | 期待値 | 補足 |
| --- | --- | --- | --- | --- | --- | --- |
| `test_snapshot_from_issue_collects_full_tree` | 正常 | Issue 起点の全系譜収集 | story Issue を起点に呼び出し | GitHub API | intake まで遡り、兄弟 epic のサブツリーを含む全 Issue が state / ラベル / assignee 付きで含まれる | - |
| `test_snapshot_from_pr_joins_issue_tree` | 正常 | PR 起点の合流 | subsystem PR を起点に呼び出し | GitHub API | `linked_issue_numbers` の subsystem Issue から Issue 起点と同じ全系譜ツリーに合流する | - |
| `test_snapshot_attaches_prs_to_issues` | 正常 | PR のぶら下げ | `open_targets` に各階層の PR + PoC PR を含めて呼び出し | GitHub API | 各 Issue ノードの配下に対応する PR（PoC PR 含む）が表示される | - |
...
| テスト名 | 正常/異常 | 概要 | 条件 | Mock | 期待値 | 補足 |
| --- | --- | --- | --- | --- | --- | --- |
| `test_load_sessions_restores_from_yaml` | 正常 | YAML からの復元 | 2 件保存済みの state.yaml | なし | `AgentSession` 2 件が復元される | - |
| `test_load_sessions_returns_empty_when_missing` | 正常 | ファイルなしは空リスト | パスが存在しない | なし | `[]` | - |
...
| テスト名 | 正常/異常 | 概要 | 条件 | Mock | 期待値 | 補足 |
| --- | --- | --- | --- | --- | --- | --- |
| `test_save_sessions_roundtrip` | 正常 | 保存 → 読み込みの往復 | 2 件を保存 | なし | `load_sessions` で同一内容が復元される | - |
| `test_save_sessions_leaves_no_tmp_file` | 正常 | アトミック書き | 保存実行後 | なし | tmp ファイルが残っていない | rename 方式の検証 |
...
| テスト名 | 正常/異常 | 概要 | 条件 | Mock | 期待値 | 補足 |
| --- | --- | --- | --- | --- | --- | --- |
| `test_build_agents_builds_all_agents` | 正常 | 全エージェント分の Agent 生成 | `LabelSettings` の全値 | なし | エージェント数分の `Agent` が生成され、確認 / 処理中ラベルが対応する値になる | - |
