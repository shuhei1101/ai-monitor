...
稼働中の設定を差し替える手段は「保持している `Settings` の中身を書き換える」ことに寄せる。
ポーリングも MCP ツールも composition root が配った同じ `Settings` を参照し続けるため、中身を書き換えれば両方に同時に効く。
差し替え用の入れ物を新設して各所の参照を張り替える方式は、参照の張り替え漏れが「片方だけ古い設定を見る」障害になるので採らない。

エージェント定義（`list[Agent]`）も同じ理由でリストの中身を入れ替える。
