...
classDiagram
  ...
  click エージェントモデル href "#エージェントモデル"
...
| エージェントモデル | `agents` | [`dict[str, AgentModel]`](#エージェントモデル) | 公開 | - | エージェント名 → モデル設定 | `{"implementer": AgentModel(model="sonnet"), ...}` | 全エージェント分のエントリ必須。欠落は起動エラー |
...
| `test_settings_when_agents_all_set` | 正常 | 全エージェント分のモデル読み込み | 全エージェント分の `agents.{name}.model` を書いた settings.yaml | なし | `settings.agents` に全件が入り、各 `AgentModel.model` が対応する値になる | - |
...
| 通知済みゲート番号 | `notified_gates` | `list[int]` | 公開 | `[]` | ユーザー確認ゲートを通知済みの番号 | `[35]` | ゲートは開いたままなので、毎周期の重複送信を防ぐために持つ（[通知](./通知.py.md)） |
...
```python
'AI_MONITOR_PROJECT=sandbox ... claude --model opus --dangerously-skip-permissions --mcp-config \'{"mcpServers":{...}}\' --append-system-prompt-file /tmp/ai-monitor-sandbox-52-architect.docs "$(cat /tmp/ai-monitor-sandbox-52-architect.prompt)"'
```
...
5. 環境変数 + `claude --model {モデル} --dangerously-skip-permissions --mcp-config '{MCP 接続先}' --append-system-prompt-file {ドキュメントファイル} "$(cat {プロンプトファイル})"` を連結して返す
...
| テスト名 | 正常/異常 | 概要 | 条件 | Mock | 期待値 | 補足 |
| --- | --- | --- | --- | --- | --- | --- |
| `test_build_launch_command` | 正常 | 起動コマンドの組み立て | フェーズ / 対応表 / Wiki を配置 | [エージェントドキュメント組み立て](./エージェントドキュメント.py.md#エージェントドキュメント組み立て) | 環境変数で始まり `claude --model` と `--append-system-prompt-file` と一時ファイルのコマンド置換を含み、ドキュメントファイルに手順書が書かれている | - |
...
| エージェントモデル | `agent_models` | [`dict[str, AgentModel]`](#エージェントモデル) | ✅ | - | エージェント名 → モデル設定 | [全体設定](#全体設定) の `agents` を渡す。全エージェント分揃っている前提（Settings 側で検証済み） |
...
| テスト用エージェントモデル | 全エージェント分の `AgentModel` を明示した辞書を生成 | fixture 名 `agent_models` |

| テスト名 | 正常/異常 | 概要 | 条件 | Mock | 期待値 | 補足 |
| --- | --- | --- | --- | --- | --- | --- |
| `test_build_agents` | 正常 | 全エージェント分の Agent 生成 | `LabelSettings` の全値・`agent_models` に全件 | なし | エージェント数分の `Agent` が生成され、確認 / 処理中ラベル・独立系フラグ・`model` が対応する値になる | - |
