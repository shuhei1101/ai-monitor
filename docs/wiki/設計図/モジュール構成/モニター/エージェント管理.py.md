...
6. 対象が PR の場合、スタック上で自分より下に open な PR が残っていれば送信を見送る（[スタック所属取得](./GitHub連携.py.md#スタック所属取得)）
   - `[INFO]` スタック下位が open のため起動を見送った（`project` / `agent_name` / 対象番号）
   - 起動しても待機して終わるだけで Claude の利用枠を消費するため、送信前に判定する
7. 対象を 1 件ずつ処理する（[1 件処理](#1-件処理)）
...
| `test_poll_when_stack_below_open` | 正常 | スタック下位が open の見送り | 対象がスタックに属し下位に open な PR あり | GitHub API / tmux | セッション作成も送信も発生しない | 下位がマージされた周期で送信される |
