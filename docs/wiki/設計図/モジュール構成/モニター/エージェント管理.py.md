...
| 論理名 | 引数名 | 型 | 必須 | デフォルト | 説明 | 補足 |
| --- | --- | --- | --- | --- | --- | --- |
| テレメトリ設定 | `telemetry` | [`TelemetrySettings \| None`](#テレメトリ設定) | ✅ | - | 送出設定 | `None` ならプロジェクト名だけ返す |
...
```python
build_launch_env(settings.telemetry, project, agent, 131)
```
...
| 型 | 説明 | 補足 |
| --- | --- | --- |
| `str` | 環境変数の並び（末尾に半角スペース 1 つ） | 常に `AI_MONITOR_PROJECT` を含む |

戻り値例:

```python
"AI_MONITOR_PROJECT=sandbox CLAUDE_CODE_ENABLE_TELEMETRY=1 ... OTEL_RESOURCE_ATTRIBUTES=ai_monitor.project=sandbox,ai_monitor.agent=single-scenario-writer,ai_monitor.number=131 "
```

#### 処理

1. `AI_MONITOR_PROJECT` にプロジェクト名を入れる（MCP がヘッダ経由で読む対象の識別子）
...
| `test_build_launch_env_when_telemetry_unset` | 正常 | テレメトリなし | `telemetry=None` | なし | `AI_MONITOR_PROJECT` だけが返り `OTEL_` を含まない | 本番の既定 |
...
3. 送信文を組み立てて send-keys で送信する（[キー送信](./tmux連携.py.md#キー送信)。[スナップショット組み立て](#スナップショット組み立て)の出力を添付）
   - 新規セッションの場合、shell に対して `{起動環境変数}claude --model {agent.model} --dangerously-skip-permissions "{スキル起動文字列 + スナップショット}"` を送信文にする（[起動環境変数](#起動環境変数)・[スキル起動文字列](#スキル起動文字列)）
