...
| 共通 | エージェントモデル | `shared/settings.py` | データモデル | [`AgentModel`](#エージェントモデル) | エージェント別のモデル設定 1 件分 | `Settings.agents` の値 |
| 共通 | テレメトリ設定 | `shared/settings.py` | データモデル | [`TelemetrySettings`](#テレメトリ設定) | エージェントの telemetry 送出設定 | 未設定なら送出しない |
| 共通 | エージェント名一覧 | `shared/settings.py` | 定数 | [`_AGENT_NAMES`](#エージェント名一覧) | 全 17 エージェント名のタプル | 設定の網羅性検証・build_agents のループの出所 |
...
| 共通 | テレメトリ環境変数 | `features/agents/service.py` | 関数 | [`build_telemetry_env`](#テレメトリ環境変数) | claude 起動コマンドに前置する環境変数列を組み立てる | 設定がなければ空文字 |
...
│   └── settings.py        # Settings / LabelSettings / MonitoredProject / AgentModel / _AGENT_NAMES
└── features/
    ├── agents/
    │   ├── types.py       # Agent
...
classDiagram
  ...
  class テレメトリ環境変数 {
    <<function>>
    +テレメトリ環境変数(テレメトリ設定, プロジェクト, エージェント定義, 主番号) str
  }
  class 監視対象プロジェクト {
    +名前: str
    +リポジトリ: str
  ...
  click テレメトリ環境変数 href "#テレメトリ環境変数"
...
| Sub-issue 総数 | `sub_issues_total` | `int` | 公開 | `0` | Sub-issue リンクの子 Issue 総数 | `2` | intake 自動クローズの判定材料 |
| Sub-issue 完了数 | `sub_issues_completed` | `int` | 公開 | `0` | closed の子 Issue 数 | `2` | 応答の `sub_issues_summary` 由来 |
...
| エージェントモデル | `agents` | [`dict[str, AgentModel]`](#エージェントモデル) | 公開 | - | エージェント名 → モデル設定 | `{"implementer": AgentModel(model="sonnet"), ...}` | 全 17 エージェント分のエントリ必須。欠落は起動エラー |
...
| `test_settings_when_agents_all_set` | 正常 | 全エージェント分のモデル読み込み | 全 17 エージェント分の `agents.{name}.model` を書いた settings.yaml | なし | `settings.agents` に全 17 件が入り、各 `AgentModel.model` が対応する値になる | - |
...
エージェント別のモデル設定 1 件分（`pydantic.BaseModel`）。
`Settings.agents` の値として使う。

### プロパティ

| 論理名 | プロパティ名 | 型 | 可視性 | デフォルト | 説明 | 例 | 補足 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| モデル | `model` | `str` | 公開 | - | 起動時の `claude --model` に渡す値 | `"claude-opus-4-7"` / `"sonnet"` | 完全 ID / 短縮名のどちらでも可・空文字は禁止（`min_length=1` で検証） |
...
| テスト名 | 正常/異常 | 概要 | 条件 | Mock | 期待値 | 補足 |
| --- | --- | --- | --- | --- | --- | --- |
| `test_agent_model_when_empty` | 異常 | 空文字禁止 | `model=""` で生成 | なし | `ValidationError` | 空値でお茶を濁さない |

### 補足

- 将来の拡張余地（reasoning effort / permission mode 等）に備えて Pydantic `BaseModel` で持つ
- フォールバック値は設けない（設定漏れをデフォルトで補うと不具合に気づけないため）
...
- 送出する signal と冗長度は [テレメトリ環境変数](#テレメトリ環境変数) が固定で決める（E2E の調査用途では全て有効にする）
...
| モデル | `model` | `str` | 公開 | - | 起動時の `claude --model` に渡す値 | `"claude-opus-4-7"` | [Settings](#全体設定) の `agents[name].model` を注入（フォールバックなし・欠落は起動時に検出） |
| 独立系 | `standalone` | `bool` | 公開 | `False` | 個別解放の対象か | `false` | `epic-poc-runner` / `library-poc-runner` / `resetter` / `quick-implementer` / `questioner` で `True` |
...
| 論理名 | 引数名 | 型 | 必須 | デフォルト | 説明 | 補足 |
| --- | --- | --- | --- | --- | --- | --- |
| テレメトリ設定 | `telemetry` | [`TelemetrySettings \| None`](#テレメトリ設定) | ✅ | - | 送出設定 | `None` なら空文字を返す |
| プロジェクト | `project` | [`MonitoredProject`](#監視対象プロジェクト) | ✅ | - | 監視対象プロジェクト | 属性 `ai_monitor.project` の値 |
| エージェント定義 | `agent` | [`Agent`](#エージェント定義) | ✅ | - | 対象エージェント | 属性 `ai_monitor.agent` の値 |
| 主番号 | `number` | `int` | ✅ | - | 対象の Issue / PR 番号 | 属性 `ai_monitor.number` の値 |

引数例:

```python
build_telemetry_env(settings.telemetry, project, agent, 131)
```
...
| 型 | 説明 | 補足 |
| --- | --- | --- |
| `str` | 環境変数の並び（末尾に半角スペース 1 つ） | 設定が `None` のときは空文字 |

戻り値例:

```python
"CLAUDE_CODE_ENABLE_TELEMETRY=1 OTEL_LOGS_EXPORTER=otlp ... OTEL_RESOURCE_ATTRIBUTES=ai_monitor.project=sandbox,ai_monitor.agent=single-scenario-writer,ai_monitor.number=131 "
```

#### 処理

1. `telemetry` が `None` の場合、空文字を返す
2. 送出を有効にする変数を並べる
...
3. 記録内容を広げる変数を並べる（`OTEL_LOG_USER_PROMPTS` / `OTEL_LOG_ASSISTANT_RESPONSES` / `OTEL_LOG_TOOL_DETAILS` / `OTEL_LOG_TOOL_CONTENT` を全て `1`）
4. `OTEL_RESOURCE_ATTRIBUTES` に `ai_monitor.project` / `ai_monitor.agent` / `ai_monitor.number` をカンマ区切りで並べる
5. 全体を半角スペース区切りで連結し、末尾にも半角スペースを付けて返す
...
| テスト名 | 正常/異常 | 概要 | 条件 | Mock | 期待値 | 補足 |
| --- | --- | --- | --- | --- | --- | --- |
| `test_build_telemetry_env` | 正常 | 環境変数の組み立て | 設定あり | なし | 有効化・エクスポーター・エンドポイントの各変数が含まれる | - |
| `test_build_telemetry_env_when_unset` | 正常 | 設定なし | `telemetry=None` | なし | 空文字 | 本番では何も前置しない |
| `test_build_telemetry_env_when_resource_attributes` | 正常 | 識別子の埋め込み | `project.name="sandbox"`・`agent.name="single-scenario-writer"`・`number=131` | なし | `OTEL_RESOURCE_ATTRIBUTES` に 3 属性がカンマ区切りで入る | 番号からエージェントの行動を引く起点 |
...
1. セッションを解決する（[検索](#検索)。主番号または監視面に `target.number` を含むセッション）
   ...
   - `[INFO]` エージェントセッションを新規作成した（`project` / `agent_name` / `number` / `session_name` / `model`）
...
3. 送信文を組み立てて send-keys で送信する（[キー送信](./tmux連携.py.md#キー送信)。[スナップショット組み立て](#スナップショット組み立て)の出力を添付）
   - 新規セッションの場合、shell に対して `{テレメトリ環境変数}claude --model {agent.model} --dangerously-skip-permissions "{スキル起動文字列 + スナップショット}"` を送信文にする（[テレメトリ環境変数](#テレメトリ環境変数)・[スキル起動文字列](#スキル起動文字列)）
   - 既存セッションの場合、稼働中の claude への入力として再開の定型文を送信文にする（モデルとテレメトリは新規起動時のみ効くため、切り替えたい場合はセッションを kill してから再作成する）
...
| `test_process_one_when_new_session` | 正常 | 新規セッションの起動コマンド | 台帳に未登録 | GitHub API / tmux | 送信文に `claude --model {agent.model}` が含まれる | - |
| `test_process_one_when_telemetry_set` | 正常 | テレメトリ環境変数の前置 | 台帳に未登録 + `telemetry` あり | GitHub API / tmux | 送信文が環境変数で始まり、`claude` の前に対象の識別子が入る | - |
| `test_process_one_when_telemetry_unset` | 正常 | テレメトリなしの起動コマンド | 台帳に未登録 + `telemetry=None` | GitHub API / tmux | 送信文が `claude` で始まる | 本番の既定 |
...
全エージェントの[エージェント定義](#エージェント定義)を[ラベル設定](#ラベル設定)と[エージェントモデル](#エージェントモデル)の値から組み立てる。
...
| エージェントモデル | `agent_models` | [`dict[str, AgentModel]`](#エージェントモデル) | ✅ | - | エージェント名 → モデル設定 | [全体設定](#全体設定) の `agents` を渡す。全 17 エージェント分揃っている前提（Settings 側で検証済み） |

引数例:

```python
build_agents(labels, agent_models=settings.agents)
```
...
```python
[Agent(name="epic-conductor", confirm_label="確認:epic-conductor", processing_label="処理中:epic-conductor", model="claude-opus-4-7"), ...]
```

#### 処理

1. [`_AGENT_NAMES`](#エージェント名一覧) を順に回し、確認 / 処理中ラベルを `labels` から取り出す（独立系 5 種は `standalone=True` にする）
2. `agent_models[name].model` の値をそのまま `Agent.model` に注入する（フォールバックなし）
3. `Agent` を組み立て、一覧で返す
...
| 例外名 | 発生条件 | メッセージ | 補足 |
| --- | --- | --- | --- |
| `KeyError` | `agent_models` にエージェント名のエントリがない | 該当エージェント名 | Settings 側の網羅性検証をすり抜けたケース（辞書を手渡しした場合の保険） |
...
| テスト用エージェントモデル | 全 17 エージェント分の `AgentModel` を明示した辞書を生成 | fixture 名 `agent_models` |

| テスト名 | 正常/異常 | 概要 | 条件 | Mock | 期待値 | 補足 |
| --- | --- | --- | --- | --- | --- | --- |
| `test_build_agents` | 正常 | 全エージェント分の Agent 生成 | `LabelSettings` の全値・`agent_models` に全 17 件 | なし | エージェント数分の `Agent` が生成され、確認 / 処理中ラベル・独立系フラグ・`model` が対応する値になる | - |
| `test_build_agents_when_missing_entry` | 異常 | 辞書欠落は KeyError | `agent_models` から 1 件抜いて呼び出し | なし | `KeyError`（欠落エージェント名） | Settings 側の検証をすり抜けた場合の保険 |
...
| セッション台帳 | `registry` | [`SessionRegistry`](#セッション台帳) | ✅ | - | 起動中セッションの台帳 | キーワード引数 |
| 前周期一覧 | `prev_targets` | `dict[str, list[MonitorTarget]]` | ✅ | - | プロジェクト名 → 前周期の open 対象一覧 | epic 一括解放の差分材料。初回は `{}` |
| 前回 heartbeat 時刻 | `last_heartbeat_at` | `str` | ✅ | - | 前回のタイムアウト検知の実行時刻（ISO 8601） | - |

引数例:

```python
run_cycle(settings, agents, registry=registry, prev_targets={}, last_heartbeat_at="2026-07-20T00:00:00+09:00")
```
...
3. クリーンアップ検知を実行する（[intake 自動クローズ](./クリーンアップ.py.md#intake-自動クローズ)・[epic 一括解放](./クリーンアップ.py.md#epic-一括解放)・[個別解放](./クリーンアップ.py.md#個別解放)）
4. 前回 heartbeat から `heartbeat_interval_sec` 以上経過していれば[タイムアウト回収](./クリーンアップ.py.md#タイムアウト回収)を実行し、heartbeat 時刻を更新する
...
- `main()` は 設定読込 → 台帳復元 → `build_agents(labels, agent_models=settings.agents)` → FastAPI アプリの生成（[アプリ生成](./HTTP受信.py.md#アプリ生成)）→ `uvicorn.run`（`127.0.0.1:{settings.port}`）で起動する。
