...
全エージェントの差分はラベル・名前というデータのみで、worker / conductor はワークフロー側の概念としてだけ存在する（モニターは区別しない）。
...
└── features/
    ├── agents/
    │   ├── types.py       # Agent
    │   └── service.py     # poll / build_launch_prompt / build_context_snapshot
    └── sessions/
...
classDiagram
  ...
  class 起動環境変数 {
    <<function>>
    +起動環境変数(テレメトリ設定, プロジェクト, エージェント定義, 主番号) str
  }
  class 監視対象プロジェクト {
    +名前: str
    +リポジトリ: str
...
| 確認ラベル接頭辞 | `confirm_prefix` | [`LabelName`](#ラベル名) | 公開 | - | `AI_MONITOR_LABEL_CONFIRM_PREFIX` の値 | `"確認:"` | [epic 一括解放](./クリーンアップ.py.md#epic-一括解放)の残存判定が使う |
...
エージェントドキュメント（フェーズ + 参考資料 + Wiki 索引）・対象番号・コンテキストスナップショットを 1 本のテキストにする。

#### 引数

| 論理名 | 引数名 | 型 | 必須 | デフォルト | 説明 | 補足 |
| --- | --- | --- | --- | --- | --- | --- |
| エージェント定義 | `agent` | [`Agent`](#エージェント定義) | ✅ | - | 対象エージェント | 名前でフェーズ設定を引く |
| 主番号 | `number` | `int` | ✅ | - | 対象の Issue / PR 番号 | - |
| 対象プロジェクト | `project` | [`MonitoredProject`](#監視対象プロジェクト) | ✅ | - | 参考資料と Wiki 索引の在り処 | - |
| スナップショット | `snapshot` | `str` | ✅ | - | コンテキストスナップショット | [スナップショット組み立て](#スナップショット組み立て)の戻り値 |

引数例:

```python
build_launch_prompt(agent, 52, project, snapshot)
```
...
1. エージェントドキュメントを組み立てる（[エージェントドキュメント組み立て](./エージェントドキュメント.py.md#エージェントドキュメント組み立て)）
2. 役割の宣言・`## 入力`（対象番号）・ドキュメント・スナップショットの順に連結して返す
...
| テスト名 | 正常/異常 | 概要 | 条件 | Mock | 期待値 | 補足 |
| --- | --- | --- | --- | --- | --- | --- |
| `test_build_launch_prompt` | 正常 | 起動プロンプトの組み立て | フェーズ / 対応表 / Wiki を配置 | なし | エージェント名・対象番号・フェーズ本文・参考資料・スナップショットが含まれる | - |
...
| エージェント定義 | `agent` | [`Agent`](#エージェント定義) | ✅ | - | 対象エージェント | モデルの指定に使う |
| 主番号 | `number` | `int` | ✅ | - | 対象の Issue / PR 番号 | - |
...
```python
'AI_MONITOR_PROJECT=sandbox ... claude --model opus --dangerously-skip-permissions "$(cat /tmp/ai-monitor-sandbox-52-architect.prompt)"'
```

#### 処理

1. 環境変数の並びを組み立てる（[起動環境変数](#起動環境変数)）
2. 起動プロンプトを組み立てる（[起動プロンプト](#起動プロンプト)）
3. 起動プロンプトをセッション名で決まる一時ファイルへ書き出す
   - 数万文字を send-keys の引数に直接埋めず、コマンド置換で渡す（本文のバッククォート・`$` が再展開されない）
4. 環境変数 + `claude --model {モデル} --dangerously-skip-permissions "$(cat {一時ファイル})"` を連結して返す

#### 例外

| 例外名 | 発生条件 | メッセージ | 補足 |
| --- | --- | --- | --- |
| `KeyError` | YAML に当該エージェントの項目が無い | エージェント名 | [起動プロンプト](#起動プロンプト)経由 |

#### 単体テスト

| テスト名 | 正常/異常 | 概要 | 条件 | Mock | 期待値 | 補足 |
| --- | --- | --- | --- | --- | --- | --- |
| `test_build_launch_command` | 正常 | 起動コマンドの組み立て | フェーズ / 対応表 / Wiki を配置 | [エージェントドキュメント組み立て](./エージェントドキュメント.py.md#エージェントドキュメント組み立て) | 環境変数で始まり `claude --model` と一時ファイルのコマンド置換を含む | - |
...
対象プロジェクト名は必ず渡す。
MCP サーバーはモニターと同居して複数プロジェクトから叩かれるため、どのプロジェクトのセッションかを呼び出し側が名乗る必要がある（[プロジェクト解決](../MCP/GitHub操作.py.md#プロジェクト解決)がこの値をヘッダ経由で読む）。

エージェント名・主番号・待受ポートも必ず渡す。
セッション内のフックが自分の素性とモニターの宛先を知る唯一の経路で、[コンテキストリセット](../../バックエンド結合/コンテキストリセット.py.md)がこれらを body に載せる。
...
1. フックとの共有変数を並べる
   - `AI_MONITOR_PROJECT` にプロジェクト名（MCP がヘッダ経由で読む対象の識別子）
   - `AI_MONITOR_AGENT` にエージェント名・`AI_MONITOR_NUMBER` に主番号・`AI_MONITOR_PORT` に待受ポート（フックがリセット要求を送るのに使う）
...
3. 送信文を組み立てて send-keys で送信する（[キー送信](./tmux連携.py.md#キー送信)。[スナップショット組み立て](#スナップショット組み立て)の出力を添付）
   - 新規セッションの場合、起動プロンプトを一時ファイルへ書き出し、shell に対して `{起動環境変数}claude --model {agent.model} --dangerously-skip-permissions "$(cat {一時ファイル})"` を送信文にする（[起動環境変数](#起動環境変数)・[起動プロンプト](#起動プロンプト)）。
     数万文字を send-keys の引数に直接埋めず、コマンド置換で渡す（本文のバッククォート・`$` が再展開されない）
...
```python
[Agent(name="epic-conductor", confirm_label="確認:epic-conductor", processing_label="処理中:epic-conductor", model="opus"), ...]
```
