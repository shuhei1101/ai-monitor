...
セッション内のフックが自分の素性とモニターの宛先を知る唯一の経路で、[コンパクト通知](../../バックエンド結合/コンパクト通知.py.md)がこれらを body に載せる。
...
1. フックとの共有変数を並べる
   ...
   - `AI_MONITOR_AGENT` にエージェント名・`AI_MONITOR_NUMBER` に主番号・`AI_MONITOR_PORT` に待受ポート（フックがコンパクト通知を送るのに使う）
...
| `test_build_launch_env_when_hook_variables` | 正常 | フック向け変数の埋め込み | `agent.name="architect"`・`number=52`・`port=8765` | なし | `AI_MONITOR_AGENT` / `AI_MONITOR_NUMBER` / `AI_MONITOR_PORT` が含まれる | コンパクト通知の宛先と素性 |
