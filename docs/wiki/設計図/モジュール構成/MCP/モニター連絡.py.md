...
git の remote URL から監視対象プロジェクト名を解決する。
...
| 型 | 説明 | 補足 |
| --- | --- | --- |
| `str` | 監視対象プロジェクト名 | モニターのセッション台帳キー（`MonitoredProject.name`）と一致する |
...
1. `git remote get-url origin` で remote URL を取得して `owner/name` にパースする
2. settings.yaml の `projects` から `repo` が一致するプロジェクトの `name` を返す（未登録リポジトリは `owner/name` をそのまま返す）
...
| テスト名 | 正常/異常 | 概要 | 条件 | Mock | 期待値 | 補足 |
| --- | --- | --- | --- | --- | --- | --- |
| `test_resolve_project` | 正常 | 登録リポジトリの名前解決 | settings.yaml に repo 一致のプロジェクトあり | git CLI | `MonitoredProject.name` を返す | - |
| `test_resolve_project_when_unregistered` | 正常 | 未登録リポジトリのフォールバック | settings.yaml に repo 一致なし | git CLI | `owner/name` を返す | - |
