...
version:
  - number: 1.0.0
    changes: 初版
  - number: 1.1.0
    changes: 冒頭リード・概要を初回の内容のまま保つルールを追加
  - number: 1.2.0
    changes: インデックスの索引セクションを `## 分類一覧` から `## 目次` へ（規約『Wiki管理』に合わせる）
---
...
| インデックス | `## 目次` | - | 必須 | architect | 各分類別ファイルへの入口リンク。見出しと「ページ」「概要」列は規約『Wiki管理』の必須（`build_wiki_index` の探索キー） |
...
- 初回に書いた内容のまま保ち、後から情報を足さない。
  データストアの詳細は `## 概要` / `## テーブル一覧` / `## 目次` に書く（両方に書くと同じ内容を 2 箇所で持つことになる）。
...
- 初回に書いた内容のまま保ち、後から情報を足さない。
  テーブルの仕様は `## 一覧` と各テーブルのセクションに書く（両方に書くと同じ内容を 2 箇所で持つことになる）。
  対象そのものが変わったときだけ既存の記述を書き換える
...
- 初回に書いた内容のまま保ち、後から情報を足さない。
  個別のテーブル仕様は `## テーブル一覧` と各分類別ファイルに書く（両方に書くと同じ内容を 2 箇所で持つことになる）。
  対象そのものが変わったときだけ既存の記述を書き換える
