...
| `## リスク仮説` | - | 必須（方針固めで草案 → 応答ループで確定） | epic-poc-runner |
| `## 検証構成` | - | 必須（方針固めで草案 → 応答ループで確定） | epic-poc-runner |
| `## 成功条件` | - | 必須（方針固めで草案 → 応答ループで確定） | epic-poc-runner |
| `## 検証結果` | - | 必須（検証実行後に記入） | epic-poc-runner |
| `## 最小再現コード` | - | 必須（検証実行後に記入） | epic-poc-runner |
...
- 発注元の epic PR と同じ起点の Issue 番号を書く（親 epic PR の番号は base とスタック表示から辿れるため書かない）
