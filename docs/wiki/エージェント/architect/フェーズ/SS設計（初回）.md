...
`## ユースケース要件` と `## 実装分担` から自 subsystem の担当範囲を確認する。
`## 実装分担` に `未起票` の後続 subsystem が残っている場合、本 subsystem のインターフェース確定時に確定報告の対象になる（実施は「設計ページ確定」）。
...
画面ありの subsystem では、epic の全体UI設計で確定した画面方向性を前提に画面構成・フロントエンド結合を書く。

`チェックシート/設計変更影響調査.md` の前半（判定一覧の作成まで）を実施し、維持 / 修正 / 廃止の判定一覧を作る。

### 設計ページの作成

`## タスク一覧` の設計タスクを上流順（インターフェース → ER図 → 画面構成 → バックエンド結合 / フロントエンド結合（フロー）→ モジュール構成）に並べ、先頭の 1 ページを作成 / 更新して subsystem ブランチに commit push する。
...
- `body`: 対象ページの設計提案（確定させたい内容の要約 + 判断が割れる論点は複数案の比較表と推奨 + 承認・修正方法の案内）
