...
- 未作成の行が残っていない場合、「確定インターフェースの共有」「インターフェース確定報告の Resolve」「ラベル除去」「作業完了報告」だけを実行する（「subsystem PR の作成」と「サブシステム一覧の更新」は実行しない）
...
同じ subsystem について確定報告が 2 回目以降で、内容が前回から変わっている場合、その前提で着手済みの subsystem PR へ確定内容を伝える。

対象は `## サブシステム一覧` の `対応 subsystem` に番号が入っていて、まだマージされていない PR のうち、確定内容が前提を変えるもの。

MCP `comment` を呼ぶ:
- `number`: 対象の subsystem PR 番号
- `is_pr`: true
- `sender`: `story-conductor`
- `receiver`: `subsystem-conductor`
- `format`:
  - `type`: `plain`
  - `body`: 確定したインターフェースと、前回共有した内容からの差分

- 確定報告が 1 回目の場合、または内容が前回から変わっていない場合、本手順は実行しない
