...
MCP `worktree_create`（`branch`: `{type}/epic/{ドメイン}`・`base_ref`: `origin/master`）→ MCP `create_draft_pr`（`base_branch`: `master`・`body`: `## 紐づく Issue` のみ・`labels`: `$AI_MONITOR_LABEL_LAYER_EPIC` の値）→ MCP `add_watch_targets`（作成した PR の番号）の順に呼ぶ。
