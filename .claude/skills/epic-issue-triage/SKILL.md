...
- 再開:子story起票 ラベル: !`echo "$GH_KIT_LABEL_RESUME_CREATE_STORY"`
...
- `is_pr`: false
- `remove_labels_`:
  - !`echo "$GH_KIT_LABEL_CONFIRM_EPIC_ISSUE_TRIAGE"`
  - !`echo "$GH_KIT_LABEL_PHASE_END"`
- `add_labels_`:
  - !`echo "$GH_KIT_LABEL_CONFIRM_EPIC_PR_INITIALIZER"`
...
- `remove_labels_`:
  - !`echo "$GH_KIT_LABEL_CONFIRM_EPIC_ISSUE_TRIAGE"`
  - !`echo "$GH_KIT_LABEL_RESUME_CREATE_STORY"`
  - !`echo "$GH_KIT_LABEL_PHASE_END"`
- `add_labels_`:
  - （なし。役割を終えるので次ラベルなし）
