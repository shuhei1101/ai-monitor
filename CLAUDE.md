...
起動コマンド・送信文・コンテキストスナップショットの仕様は `設計図/インターフェース定義/バックエンド/エージェント起動検知.py.md` と `設計図/モジュール構成/モニター/エージェント管理.py.md` が SoT。
...
トリガーと動作は `設計図/インターフェース定義/バックエンド/{intake自動クローズ, epic一括解放, 個別解放}.py.md` が SoT。
...
| `インターフェース定義/バックエンド/{ツール名}.md` | 結合（integration） | `tests/integration/mcp/test_{ツール名}.py` ほか |
| `モジュール構成/{ドメイン}/{モジュール名}.md` | 単体（unit） | `tests/unit/{パッケージパス}/test_{モジュール名}.py` |

1 設計書 = 1 テストファイルの原則で、テストを新設するときはまず対応する設計書を書き、書式は各テンプレート（`テンプレート/シナリオ.md` / `テンプレート/インターフェース定義.md` / `テンプレート/モジュール構成.md`）に従う。
...
- MCP ツール詳細ファイル（`設計図/インターフェース定義/バックエンド/`）と各フェーズページへの記載は実装時に起こす

### system レイヤーの実装（設計済み・実装未了）

新規プロジェクトの立ち上げと既存プロジェクトの移行（リバースエンジニアリング）を担う `system-conductor` / `system-architect` は、設計が確定済みで実装が残っている。

- シナリオ: `設計図/シナリオ/複合ユースケース/{新規プロジェクトの立ち上げ, 既存プロジェクトのリバースエンジニアリング}.md`
- 残作業: `constants.env` / `_AGENT_NAMES` / `settings.yaml.example` / `agent_phases.yaml` への登録と、フェーズページ本体の作成（手順は `規約/エージェント追加.md`）
