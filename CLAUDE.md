...
**ユーザーは普通に Issue（= Issue）を起票**する（レイヤーを意識しない）。
`intake-issue-triager` が **Issue の内容を作業単位に分解** し、各作業について以下の基準で **サブ Issue の種別と件数** を判定する。
判定案はコメントで **ユーザーに事前確認** を取り、承認後に **Issue に Sub-issue として** epic / story / subsystem / chore Issue を作成する。

| 変更内容 | サブ Issue 種別 | 例 |
| --- | --- | --- |
| 複合ユースケースシナリオを新規 / 変更 | epic | 新機能全体追加、認証フロー刷新、ユースケース間の連鎖修正 |
| 単一ユースケースシナリオを新規 / 変更 | story | UC 内の要件追加、UC 単位のバグ修正、単一 UC の挙動変更 |
| 実装のみ変更（シナリオ不変） | subsystem | 内部リファクタ、パフォーマンス改善、実装バグ修正、ログ追加 |
| 軽微な修正 | chore | typo、コメント、ログレベル変更、コードフォーマット、依存の軽微バージョン更新 |

- **1 つのIssue から複数のサブ Issue が生まれる場合もある**（例: 1 epic + 1 chore、独立した 2 epic など）
- Issue は集約元として残し、全サブ Issue 完了後にクローズする（勝手にクローズしない）
- Issue 本文はユーザーが最初に書いた内容のまま保持（`intake-issue-triager` は本文を書き換えない）
- サブ Issue の本文骨組み・要件確定は各レイヤーの `-intake-issue-triager` が担当（`intake-issue-triager` はサブ Issue の作成までで役割終了）
- 判定の詳細フローは `docs/wiki/判定フローチャート/レイヤー.md` 参照
...
5. **要件観点調査（サブエージェント並列起動）**:
   - `gh-kit:functional-requirements-analyzer` / `gh-kit:nonfunctional-requirements-analyzer` を並列起動
...
| gh-kit:functional-requirements-analyzer | Issue 本文（概要・背景・現状） + Issue 番号 + 対象領域 | 機能要件観点リスト |
| gh-kit:nonfunctional-requirements-analyzer | Issue 本文（概要・背景・現状） + Issue 番号 + 採用予定の外部依存 | 非機能要件観点リスト |
...
### 既存プロジェクト解析 → gh-kit 移行モニター

- 既に稼働中の（gh-kit を使ってない）プロジェクトを解析し、gh-kit 準拠の Wiki 構成（モジュール構成 / データフロー / 状態遷移 / 設定 / 定数 / ER図 等）に逆生成するためのモニター群を作る
