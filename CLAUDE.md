...
- 本ファイル・`constants.env`・シナリオ（`ライブラリ選定.md` = 発注元 / `ライブラリPoC検証.md` = poc-runner）は反映済み。SKILL.md は未作成（残りエージェントの移植に含める）
...
| エージェント詳細 17 個 | 各 SKILL.md（フロー本体）+ `Claudeハーネス/エージェント一覧.md` | スキル 7/17 / 一覧未 |
...
スキル一覧 / 動的注入対応表 / フック一覧 / プラグイン構成は作らない。
